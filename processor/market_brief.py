...
def _build_prompt(region: str, headlines: List[dict], indicators: dict) -> str:
    ...
    if indicators.get('premarket_pct') is not None:
        pm = indicators['premarket_pct']
        direction = '상승' if pm > 0 else '하락'
        lines.append(f'프리장/시간외 등락(SPY, 정규장 종가 대비): {pm:+.2f}% ({direction})')
    lines.append('')
...
- 지수 방향(상승/하락/혼조)은 반드시 입력으로 준 '일일 수익률' 부호와 일치시켜라. 수익률 정보가 없으면 헤드라인 다수가 가리키는 방향을 따르되 단정 표현은 피해라.
...
def _fetch_indicators(region: str) -> dict:
    ...
    # US 프리장/시간외 등락 — 정규장 종가 대비 (참고용, 변동 큼)
    if region == 'us':
        pm = _fetch_premarket_us()
        if pm is not None:
            out['premarket_pct'] = pm
    return out


def _fetch_premarket_us() -> Optional[float]:
    """SPY 프리장/시간외 최종가 vs 직전 정규장 종가 등락%. 실패 시 None."""
    try:
        import yfinance as yf
        # 시간외 포함 1분봉 (prepost=True) — 최신 체결가
        intra = yf.download('SPY', period='1d', interval='1m',
                            prepost=True, progress=False)
        if intra is None or intra.empty:
            return None
        last = float(intra['Close'].dropna().iloc[-1])
        # 직전 정규장 종가 (prepost=False 일봉)
        daily = yf.download('SPY', period='5d', prepost=False, progress=False)
        if daily is None or daily.empty:
            return None
        prev_close = float(daily['Close'].dropna().iloc[-1])
        if prev_close <= 0:
            return None
        pct = (last / prev_close - 1.0) * 100.0
        # 너무 미미하면(±0.05% 미만) 무시 — 정규장 마감가와 동일 취급
        if abs(pct) < 0.05:
            return None
        return round(pct, 2)
    except Exception as e:
        print(f'[market_brief] 프리장 fetch 실패: {e}')
        return None
