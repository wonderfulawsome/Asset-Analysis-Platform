...
def get_region_detail(sgg_cd: str = Query(..., description='시군구 코드 5자리')):
    ...
    try:
        ...
        if r.data:
            payload = r.data[0]['payload']
            if payload:
                return payload
            print('[region-detail] cached payload empty — recomputing')
    except Exception as e:
...
def get_config():
    ...
    return {
        'vworld_api_key': (
            ...
            or os.getenv('VWORLD_API_KEY_REAL_ESTATE')
            or os.getenv('VWORLD_KEY')
            or ''
        ).strip(),
        'kakao_js_key': (os.getenv('KAKAO_JS_KEY') or '').strip(),  # 옛 KakaoMap 호환 (deprecated)
    }
...
def get_sgg_overview(ym: str = Query(default='', description='YYYYMM, 미지정 시 최신')):
    ...
    try:
        ...
        if r.data:
            ...
            if payload:
                return payload
            print('[sgg-overview] cached payload empty — recomputing')
    except Exception as e:
...
def get_market_summary():
    ...
    sd0 = data.get('signal_distribution') or {}
    has_market_data = bool(data.get('top_up') or data.get('top_down') or any(sd0.values()))

    if not has_market_data:
        summary_text = '부동산 집계 데이터가 아직 준비되지 않았습니다. 지도 영역과 지역명은 먼저 표시되며, 거래·가격 지표는 데이터 적재 후 갱신됩니다.'
