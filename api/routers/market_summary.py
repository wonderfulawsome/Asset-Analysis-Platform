...
from database.repositories import (                          # DB 조회 함수들
    ...
    fetch_macro_latest2,                                     # 최근 5건 거시지표 (결손 폴백)
...
def get_market_brief_endpoint(region: str = Query('us')):
    ...
    try:
        ...
        return {'text': '시황 데이터 수집 중', 'keywords': [],
...
def _build_home_brief(lang: str, region: str) -> str:
    """홈 화면 '오늘의 종합 판단' 카드 — 1~2문장 brief.

    데이터 3개만 사용 (사용자 요청 2026-05-13):
      1) RSI → 과매수(≥70) / 과매수 근접(≥60) / 과매도(≤30) / 과매도 근접(≤40) / 중립
      2) 지수 일일 수익률 (US: S&P500, KR: KOSPI)
      3) 펀더멘털 반영도 (noise_score) → 정렬 / 약정렬 / 분리 / 큰 분리
    명사형 종결, 자문 표현 0, 이모지 0.
    """
    region = _norm_region(region)
    is_en = (lang == 'en')
    # 최근 5건 macro 행 가져와서 null 인 값은 과거 행에서 보완 (주말/장후/지표 시차 대비)
    ...
    def _pick(field):
        ...
        return None
    rsi = _pick('sp500_rsi')
    ret = _pick('sp500_return')
    # 펀더멘털-주가 갭 — regime.get_fundamental_gap 사용 (펀더멘털 탭과 동일 데이터 source)
    gap_top_pct = None
    try:
        from api.routers.regime import get_fundamental_gap as _get_fg
        fg = _get_fg(region=region, days=2520) or {}
        gap_top_pct = (fg.get('current') or {}).get('top_pct')
    except Exception:
        gap_top_pct = None

    idx_name = 'S&P500' if region == 'us' else 'KOSPI'
    ...
    if rsi is not None:
        try:
            r = float(rsi)
            if r >= 70:
                rsi_phrase = f"RSI {r:.1f} 과매수 영역"
            elif r >= 60:
                rsi_phrase = f"RSI {r:.1f} 과매수 근접"
            elif r <= 30:
                rsi_phrase = f"RSI {r:.1f} 과매도 영역"
            elif r <= 40:
                rsi_phrase = f"RSI {r:.1f} 과매도 근접"
            else:
                rsi_phrase = f"RSI {r:.1f} 중립 영역"
        except Exception:
            rsi_phrase = "RSI 수집 중"
    else:
        rsi_phrase = "RSI 수집 중"

    # 일일 수익률 풀이 — region 별 단위 분기 (US: decimal ×100, KR: 이미 percent)
    if ret is not None:
        try:
            rv = float(ret)
            if region == 'us':
                rv = rv * 100  # decimal → percent
            ret_phrase = f"{idx_name} 일일 수익률 {rv:+.2f}%"
        except Exception:
            ret_phrase = f"{idx_name} 수익률 수집 중"
    else:
        ret_phrase = f"{idx_name} 수익률 수집 중"

    # 펀더멘털-주가 갭 → "상위 N%" 위치 (사용자 요청 2026-05-13: 펀더멘털 탭 데이터와 동일 source)
    if gap_top_pct is not None:
        try:
            tp = float(gap_top_pct)
            ns_phrase = f"펀더멘털-주가 갭 차이 상위 {tp:.1f}% 위치"
        except Exception:
