...
def upsert_macro(records: list[dict], region: str = 'us') -> None:
    """거시 지표 데이터를 macro_raw 테이블에 upsert합니다. (region+date 기준)"""
    if not records:
        return
    records = [{**r, 'region': region} for r in records]
    client = get_client()
    client.table("macro_raw").upsert(records, on_conflict="region,date").execute()
    print(f"[DB] macro_raw {len(records)}건 ({region}) upsert 완료")

# macro_raw에서 756일치를 조회하는 함수
def fetch_macro(days: int = 756, region: str = 'us') -> list[dict]:
...
# records: 주가의 딕셔너리 형태
def upsert_regime(record: dict, region: str = 'us') -> None:
    """HMM 국면 결과를 market_regime 테이블에 upsert합니다. (region+date+index_name 기준)."""
    ...
    client = get_client()
    client.table("market_regime").upsert(record, on_conflict="region,date,index_name").execute()
    print(f"[DB] market_regime {record['date']} ({record.get('index_name', 'sp500')}, {region}) upsert 완료")
...
def fetch_regime_current(index_name: str = 'sp500', region: str = 'us') -> Optional[dict]:
    client = get_client()
    response = (
...
def fetch_regime_current_all(region: str = 'us') -> list[dict]:
    result = []
    for name in ['sp500', 'ndx', 'sox']:
        r = fetch_regime_current(name, region=region)
        if r:
            result.append(r)
    return result

# 가장 최근 거시 지표 1건을 조회
def fetch_macro_latest(region: str = 'us') -> Optional[dict]:
...
def fetch_macro_latest2(region: str = 'us') -> list:
...
def fetch_regime_history(days: int = 30, index_name: str = 'sp500', region: str = 'us') -> list[dict]:
    client = get_client()
    response = (
        client.table("market_regime")
        .select("*")
...
def upsert_fear_greed(record: dict, region: str = 'us') -> None:
    record = {**record, 'region': region}
    client = get_client()
    client.table("fear_greed_raw").upsert(record, on_conflict="region,date").execute()
    print(f"[DB] fear_greed_raw {record['date']} ({region}) upsert 완료")


def fetch_fear_greed_latest(region: str = 'us') -> Optional[dict]:
    client = get_client()
    response = (
...
def fetch_fear_greed_latest2(region: str = 'us') -> list:
...
def upsert_index_prices(records: list[dict], region: str = 'us') -> None:
    """ETF 가격/등락률을 index_price_raw 테이블에 upsert합니다. (region+date+ticker 기준)"""
    if not records:
        return
    records = [{**r, 'region': region} for r in records]
    client = get_client()
    client.table("index_price_raw").upsert(records, on_conflict="region,date,ticker").execute()
    print(f"[DB] index_price_raw {len(records)}건 ({region}) upsert 완료")


def fetch_index_prices_latest(region: str = 'us') -> list[dict]:
...
def upsert_sector_macro(records: list[dict], region: str = 'us') -> None:
    """섹터 매크로 지표를 sector_macro_raw 테이블에 upsert합니다. (region+date 기준)"""
    if not records:
        return
    records = [{**r, 'region': region} for r in records]
    client = get_client()
    client.table("sector_macro_raw").upsert(records, on_conflict="region,date").execute()
    print(f"[DB] sector_macro_raw {len(records)}건 ({region}) upsert 완료")


def fetch_sector_macro_history(limit: int = 120, region: str = 'us') -> list[dict]:
...
def upsert_sector_cycle(record: dict, region: str = 'us') -> None:
    """경기국면 분석 결과를 sector_cycle_result 테이블에 upsert합니다. (region+date 기준)"""
    record = {**record, 'region': region}
    ...
    client = get_client()
    client.table("sector_cycle_result").upsert(record, on_conflict="region,date").execute()
    print(f"[DB] sector_cycle_result {record['date']} ({region}) upsert 완료")
...
def fetch_sector_cycle_latest(region: str = 'us') -> Optional[dict]:
...
def fetch_sector_cycle_history(days: int = 12, region: str = 'us') -> list[dict]:
...
def upsert_noise_regime(record: dict, region: str = 'us') -> None:
    """Noise vs Signal 국면 결과를 noise_regime 테이블에 upsert합니다. (region+date 기준)"""
    record = {**record, 'region': region}
    ...
    client = get_client()
    try:
        client.table("noise_regime").upsert(record, on_conflict="region,date").execute()
        ...
        if 'schema cache' in str(e) or 'column' in str(e).lower():
            ...
            for key in ('feature_contributions', 'feature_values'):
                record.pop(key, None)
            client.table("noise_regime").upsert(record, on_conflict="region,date").execute()
            print(f"[DB] noise_regime {record['date']} ({region}) upsert 완료 (JSONB 컬럼 미존재, 기본 필드만 저장)")
            return
        raise
    print(f"[DB] noise_regime {record['date']} ({region}) upsert 완료")


def fetch_noise_regime_current(region: str = 'us') -> Optional[dict]:
...
def upsert_crash_surge(record: dict, region: str = 'us') -> None:
    """폭락/급등 전조 결과를 crash_surge_result 테이블에 upsert합니다. (region+date 기준)"""
    record = {**record, 'region': region}
    ...
    client = get_client()
    try:
        client.table("crash_surge_result").upsert(record, on_conflict="region,date").execute()
    except Exception as e:
        if 'schema cache' in str(e) or 'column' in str(e).lower():
            for key in ('shap_values', 'feature_importance', 'feature_values'):
                record.pop(key, None)
            client.table("crash_surge_result").upsert(record, on_conflict="region,date").execute()
            print(f"[DB] crash_surge_result {record['date']} ({region}) upsert 완료 (JSONB 컬럼 미존재, 기본 필드만 저장)")
            return
        raise
    print(f"[DB] crash_surge_result {record['date']} ({region}) upsert 완료")


def fetch_crash_surge_current(region: str = 'us') -> Optional[dict]:
...
def _fetch_all_pages(table: str, select: str, order_col: str = "date",
    ...
    region 인자가 있으면 .eq('region', region) 필터 적용.
    """
    client = get_client()
    all_data = []
    page_size = 1000
    ...
    while True:
        ...
        q = q.order(order_col, desc=False).range(offset, offset + page_size - 1)
        response = q.execute()
        batch = response.data
...
def fetch_crash_surge_all(region: str = 'us') -> list[dict]:
    """전체 crash/surge 히스토리를 날짜 오름차순으로 조회합니다. (방향성 분석용)"""
    return _fetch_all_pages(
        "crash_surge_result",
        "date,crash_score,surge_score,net_score",
        region=region,
    )


def fetch_macro_closes(region: str = 'us') -> list[dict]:
...
def upsert_chart_predict(record: dict, region: str = 'us') -> None:
    """앙상블 예측 결과를 chart_predict_result 테이블에 upsert합니다. (region+date+ticker 기준)"""
    record = {**record, 'region': region}
    for key in ('actual', 'predicted'):
        if isinstance(record.get(key), list):
            record[key] = json.dumps(record[key], ensure_ascii=False)
    client = get_client()
    client.table("chart_predict_result").upsert(record, on_conflict="region,date,ticker").execute()
    print(f"[DB] chart_predict_result {record['date']} ({record['ticker']}, {region}) upsert 완료")


def fetch_chart_predict(ticker: str, region: str = 'us') -> Optional[dict]:
...
_REGION_SUMMARY_INT_COLS = {
    "trade_count", "avg_price", "median_price",
    "jeonse_count", "wolse_count", "avg_deposit", "population",
}
...
def upsert_region_summary(records: list[dict]) -> None:
    if not records:
        return
    import math
    # NaN/pd.NA → None + INT 컬럼 float→int 캐스팅 (PG INTEGER 가 "52515.0" 거부)
    cleaned = []
    for r in records:
        row = {}
        for k, v in r.items():
            ...
            try:
                ...
                pass
            if k in _REGION_SUMMARY_INT_COLS and isinstance(v, float):
                row[k] = int(v)
            else:
                row[k] = v
        cleaned.append(row)
    ...
    client.table("region_summary").upsert(
        cleaned, on_conflict="stdg_cd,stats_ym"
    ).execute()
    print(f"[DB] region_summary {len(cleaned)}건 upsert 완료")
...
def upsert_sector_valuation(records: list[dict], region: str = 'us') -> None:
    """섹터 ETF 의 PER/PBR upsert (region+date+ticker)."""
    if not records:
        return
    records = [{**r, 'region': region} for r in records]
    client = get_client()
    client.table("sector_valuation").upsert(
        records, on_conflict="region,date,ticker"
    ).execute()
    print(f"[DB] sector_valuation {len(records)}건 ({region}) upsert 완료")


def fetch_sector_valuation_history(days: int = 365 * 5, region: str = 'us') -> list[dict]:
...
def upsert_valuation_signal(record: dict, region: str = 'us') -> None:
    """시장 밸류 1행 upsert (region+date UNIQUE)."""
    if not record:
        return
    record = {**record, 'region': region}
    client = get_client()
    client.table("valuation_signal").upsert(
        record, on_conflict="region,date"
    ).execute()


def upsert_valuation_signal_bulk(records: list[dict], region: str = 'us') -> None:
    """다행 일괄 upsert."""
    if not records:
        return
    records = [{**r, 'region': region} for r in records]
    ...
    for i in range(0, len(records), 200):
        client.table("valuation_signal").upsert(
            records[i:i + 200], on_conflict="region,date"
        ).execute()


def fetch_valuation_signal_latest(region: str = 'us') -> dict | None:
    """가장 최근 1건. 없으면 None."""
    client = get_client()
    try:
        r = (
