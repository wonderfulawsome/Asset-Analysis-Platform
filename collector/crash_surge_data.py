...
def _fetch_fred(series_id: str, col_name: str, retries: int = 3,
    ...
    """FRED 공식 API로 데이터 수집 (API 키 있으면 JSON API, 없으면 CSV fallback).
    ...
    for attempt in range(retries):
        try:
            if FRED_API_KEY:
                ...
                resp = _fred_session.get(FRED_API_BASE, params=params, timeout=timeout)
                resp.raise_for_status()
                data = resp.json()['observations']
                rows = [(r['date'], r['value']) for r in data if r['value'] != '.']
                ...
                resp = _fred_session.get(url, timeout=timeout)
                resp.raise_for_status()
...
def fetch_crash_surge_light(lookback_days: int = 300) -> dict:
    ...
    # 4) Cboe 5개 — 장 중이면 실시간 가격 포함 (실제 장중가격은 regularMarketPrice 라는 딕셔너리 키 사용)
    cboe = {}  # Cboe 지수 저장용
