  ...
  return (
    <TerminalSection title="종합 신호" right={`점수 ${score >= 0 ? "+" : ""}${score.toFixed(1)}`}>
      ...
      <div className="grid grid-cols-5 gap-1">
        <Cell label="거래" score={signal.trade_score} pct={signal.feature_breakdown?.trade_chg_pct} />
        <Cell label="가격" score={signal.price_score} pct={signal.feature_breakdown?.price_mom_pct} />
        <Cell label="인구" score={signal.pop_score}   pct={signal.feature_breakdown?.pop_chg_pct}   />
        <Cell label="금리" score={signal.rate_score ?? null}
              extra={signal.feature_breakdown?.base_rate != null
                ? `${signal.feature_breakdown.base_rate.toFixed(2)}` : null} />
        <Cell label="이동" score={signal.flow_score ?? null}
