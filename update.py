...
# ═══════════════════════════════════════════════════════════════════════════════
# [117] 2026-05-08 (UTC) — 이상 탐지 탭 UX 리워드 ("평소 이탈도" 프레이밍)
# ═══════════════════════════════════════════════════════════════════════════════
#
# [개요]
# 이상 탐지 탭의 모든 사용자 노출 문구를 "시장 평소 이탈도" 프레이밍으로 통일.
# 통계/학술 jargon (D², percentile, μ,Σ, 분위수, 드문 영역) 을 일상어 + 살짝 전문
# 톤 (평소 이탈도, 과거 10년 기준 위치, 평소와의 거리, 비교 일수, 평소와 다름) 으로
# 교체. 알고리즘·데이터 흐름은 그대로 — 라벨/라벨링 함수만 갱신.
#
# [왜]
# - 스크린샷 기반 사용자 피드백: "98.4%" + "최상위 5% / 드문 영역" + 수식 노출이
#   직관성 부족 (특히 "드문 영역" → 자동으로 "위험" 으로 해석되는 인지 편향).
# - 자문 리스크 회피 (CLAUDE.md 신규 섹션) 와 직관성을 동시에 잡으려면 강도 표현은
#   유지하되 학술 단어를 제거 + "평소 vs 다름" 한 축으로 일관화.
# - 수식 (Σ⁻¹) 직접 노출 → 일상어 한 문단으로 대체. 의미 보존.
#
# [수정 파일]
#   1. static/js/i18n.js
#      KO:
#        'tab.signal'         : '이상 탐지'        → '평소 이탈도'
#        'an.cardLabel'       : '시장 이상 탐지'    → '시장 평소 이탈도'
#        'an.chartLabel'      : '10년 이상도 추이'  → '10년 평소 이탈도 추이'
#        'an.contribsLabel'   : '무엇이 이상한가'   → '무엇이 평소와 다른가'
#        'an.disclaimer'      : '과거 분포와의 거리만 측정합니다. 미래 예측이 아니며,
#                                매수·매도 권유가 아닙니다.'
#                              → '과거 데이터 기준의 거리 측정이며, 미래 방향을
#                                 예측하지 않습니다. 매수·매도 권유가 아닙니다.'
#        'tile.anomaly'       : '이상 탐지'        → '평소 이탈도'
#        'tile.anomalySub'    : '과거 대비 이상도'  → '과거 평소와의 거리'
#      EN: 동일 의도 매핑 ("Deviation From Normal", "Distance From Normal" 등)
#
#   2. static/js/anomaly.js
#      percentileLabel(pct) — 5단계 라벨 전면 교체:
#        99%+/95%+/80%+/50%+/20%+/<20%
#          → 평소와 매우 다름 / 평소와 다름 / 평소보다 높은 편 / 평소에 가까움 /
#            평소에 매우 가까움  (강도만, 방향성 0)
#      renderSummary 헤더·게이지·stat·하단 설명:
#        "10년 분포 내 위치 (percentile)"   → "과거 10년 기준 위치"
#        gauge labels "매우 평온/중앙/드문 영역"  → "평소에 가까움/보통/평소와 다름"
#        "이상도 D²"                         → "평소와의 거리"
#        "최근 90일 분위수"                  → "최근 90일 기준 위치"
#        "표본 (μ,Σ 추정)"                   → "비교 일수"
#        하단 수식 (D² = (x−μ)ᵀΣ⁻¹(x−μ))
#          → "여러 시장 지표를 한 번에 비교해, 현재 시장이 과거 10년의 평소
#             모습과 얼마나 다른지 계산합니다.\n과거 데이터 기준의 거리 측정이며,
#             미래 방향을 예측하지 않습니다."
#      renderChart 범례: "D² (sqrt 스케일, 시계열)" → "평소와의 거리 (시계열)"
#      renderContribs 안내문: "오늘 D² = 각 피처 기여의 합" →
#        "오늘의 평소 이탈도 = 각 지표 기여의 합"
#      renderKnn:
#        "10차원 거리"        → "평소 패턴 거리"
#        "10차원 좌표 ... Mahalanobis pairwise" → "시장 지표 조합 ... 강도와 방향
#         모두 매칭" (학술용어 제거, 의미 보존)
#
#   3. templates/stocks.html
#      tab-signal main 의 fallback 라벨 3개 (an.cardLabel/an.chartLabel/
#      an.contribsLabel) 도 i18n 키와 동일 문구로 교체. 주석 "신호 탭" → "평소
#      이탈도 탭".
#      anomaly.js 캐시 버스트: ?v=1 → ?v=2.
#
# [신규 파일]
#   없음 — 텍스트만 변경. 데이터·API·DB·계산 로직 변동 0.
#
# [자문 리스크 가드]
# - "확률·가능성·매수·매도·위험·전망" 단어 0개 (탭 전체 grep).
# - percentile 5단계 라벨 모두 "평소 vs 다름" 한 축 — 방향성 (상승/하락/위험) 표현
#   금지.
# - 하단 disclaimer "과거 데이터 기준의 거리 측정이며, 미래 방향을 예측하지 않습
#   니다." 한 문장이 자문 회피의 유일한 의미 가드 — 절대 제거 금지.
#
# [검증]
# - 파일 3종 라벨 grep: 잔존 "이상도/이상 탐지/D²/percentile/Σ" 없음 (i18n.js
#   변수명 _Z 제외 — 사용자 노출 X).
# - anomaly.js?v=2 캐시 버스트 적용 → 클라이언트 강제 재로드.
# - 라벨 함수 percentileLabel 의 5단계 모두 "평소 vs 다름" 톤으로 통일 — 사용자
#   화면 어디에도 "위험/안전" 단어 없음.
# - Launch preview 패널에서 templates/stocks.html 갱신 확인.


# ═══════════════════════════════════════════════════════════════════════════════
# [118] 2026-05-08 (UTC) — 평소 이탈도 탭 카드 재배치 + k-NN 시기별 시장 이벤트
# ═══════════════════════════════════════════════════════════════════════════════
#
# [개요]
# (1) "시장 평소 이탈도" 메인 점수 카드 (큰 게이지/3 stat/일상어 설명) 제거.
# (2) 카드 순서 재배치: ① 10년 평소 이탈도 추이 차트 (맨 위) → ② 과거 같은 패턴
#     (k-NN) → ③ 무엇이 평소와 다른가 (top contributors) → ④ AI 해설 → 면책.
# (3) k-NN 카드의 각 과거 시점에 그 시기의 시장 이벤트 라벨 표시 (📌 이모지).
#
# [왜]
# - 사용자 요청: "시장 이상 탐지 카드는 없애고 과거 시장 이상도 추이 카드를 맨 위로,
#   과거 같은 패턴 카드를 2번째로". 메인 점수가 percentile 게이지 + 3 stat 으로
#   장식적이라 차트 단독 + 비슷했던 시점 + 무엇이 다른가 의 3-단 정보 흐름이
#   더 직관적.
# - "그 당시 어떤 뉴스가 있었던 시기인지" — k-NN 결과에 날짜만 있으면 사용자가
#   "2008-10-13 이 뭐였더라" 검색해야 하는 마찰. 큐레이팅된 시장 이벤트 라벨로
#   즉시 맥락 제공.
#
# [수정 파일]
#   1. templates/stocks.html (tab-signal main 재구성)
#      - <div class="card"><div id="an-summary"></div></div> 통째로 제거.
#      - an-chart (10년 추이) 카드 → 맨 위.
#      - an-knn (과거 같은 패턴) 카드 → 2번째.
#      - an-contribs (무엇이 평소와 다른가) 카드 → 3번째.
#      - an.disclaimer fallback 텍스트도 i18n 키와 동일하게 정정.
#      - anomaly.js?v=2 → v=3 캐시 버스트.
#
#   2. static/js/anomaly.js
#      A) MARKET_EVENTS_US 신규 추가 — 큐레이팅된 사실 사전 (해석·감정 단어 X).
#         {from, to, label} 20건. 2015-08 위안화 절하 / 2016-06 브렉시트 /
#         2018-02 Volmageddon / 2019-08 곡선역전 / 2020-02~04 코로나·서킷브레이커 /
#         2021-01 GME squeeze / 2022-02 우크라 침공 / 2022-06 CPI 9.1% / 2023-03
#         SVB·CS / 2023-10 이스라엘-하마스 / 2024-08 日 캐리트레이드 청산 / 2024-11
#         미 대선 (트럼프 재선) / 2025-04 美 상호관세 등.
#      B) findEvent(dateStr) — YYYY-MM-DD 문자열 lex 비교로 사전 lookup, 매칭
#         라벨 반환 또는 null.
#      C) loadAnomaly — sumEl 참조 제거, renderSummary 호출 제거. 카드 렌더 순서
#         차트 → k-NN → contributors 로 재배치 (의미상 독립이지만 가독성용).
#      D) renderKnn — 행 레이아웃 변경:
#         이전: 좌(date / 평소 패턴 거리)  우(#N 가까움)
#         지금: 좌(date)  우(#N 가까움 · 거리 X.XX)
#               + 그 아래 줄: 📌 이벤트 라벨 (매칭) 또는 회색 이탤릭 "기록된
#               시장 이벤트 없음".
#         안내문에 "📌 = 그 시기 주요 시장 이벤트 (사실 기록만, 결과 해석 없음)"
#         한 줄 추가 — 자문 회피 + 의미 가드.
#      E) renderSummary 함수 본체 + "// ── 1. 메인 점수 카드 ──" 주석 통째로
#         삭제 (dead code 제거). 차트 주석 인덱스 1~4 → 1~3 갱신.
#         파일 헤더 docstring 도 "4개 카드 → 3개 카드" + k-NN 이벤트 라벨 언급으로
#         수정.
#
# [신규 파일]
#   없음 — 시장 이벤트 사전은 anomaly.js 내 const 로 인라인 (별도 모듈/엔드포인트
#   불필요, 사용처가 1곳뿐이라 파일 분리 시 오히려 navigation 부담).
#
# [자문 리스크 가드]
# - 시장 이벤트 라벨은 모두 사건 사실만 ("리먼브라더스 파산", "코로나19 팬데믹",
#   "美 상호관세 발표"). "그래서 시장이 폭락했다", "이런 패턴 다음엔 반등했다"
#   같은 결과·인과·예측 단어 0개.
# - "기록된 시장 이벤트 없음" fallback — 매칭 실패 시 거짓 라벨 부착 금지.
# - 안내문에 명시: "사실 기록만, 결과 해석 없음".
#
# [확장 방법]
# - 신규 이벤트 추가: anomaly.js 의 MARKET_EVENTS_US 배열에 {from, to, label} 한
#   줄 push. 정렬 불필요 (전수 검색이지만 20건 X 5 k-NN = 100회 비교라 무시).
# - KR region 활성 시 MARKET_EVENTS_KR 배열 + region 분기 추가 필요.
# - 외부 뉴스 API 통합 시 findEvent() 만 fetch 비동기로 교체, k-NN 행 렌더는
#   기존 그대로 사용 가능 (인터페이스 유지).
#
# [검증]
# - grep "renderSummary|an-summary" anomaly.js + stocks.html → 코멘트 1줄만 잔존
#   (live 참조 0).
# - findEvent("2020-03-16") → "코로나19 팬데믹·NYSE 서킷브레이커 4회".
...
# ═══════════════════════════════════════════════════════════════════════════════
# [121] 2026-05-08 (UTC) — 한자 제거 + k-NN 다양화 윈도 30→90일
# ═══════════════════════════════════════════════════════════════════════════════
#
# [개요]
# (1) MARKET_EVENTS_US 라벨에서 한자 (美/日/英/對/中) 전수 제거 → 한글 정자.
# (2) KNN_DIVERSITY_DAYS 30→90 — 매칭 시점 간 최소 3개월 간격 보장.
#
# [왜]
# - 사용자 피드백: "한자는 안나오게" + "각 이벤트들이 너무 따닥 붙어있음, 최소
#   3개월 차이". 이전 매칭 (2025-02-06/2024-12-23/2024-11-15) 은 모두 4개월 안짝
#   클러스터.
# - 한자: 일관된 한글 표기로 가독성 ↑, CJK 폰트 누락 환경 방어.
# - 다양화 90일: "비슷했던 과거 시점" 의 의미가 같은 분기 내 클러스터링이 아닌
#   서로 다른 episode 로 보장.
#
# [수정 파일]
#   1. processor/feature_anomaly.py
#      - KNN_DIVERSITY_DAYS = 30 → 90 (상수 1줄). 주석에 "사용자 요청" 명시.
#
#   2. static/js/anomaly.js
#      MARKET_EVENTS_US 라벨 한자→한글 (8건):
#        美 2년/10년       → 미국 2년/10년
#        美 국채금리 급등   → 미국 국채금리 급등
#        美 CPI 4~5%       → 미국 CPI 4~5%
#        美 CPI 9.1%       → 미국 CPI 9.1%
#        英 미니예산 위기  → 영국 미니예산 위기
#        日 캐리트레이드   → 일본 캐리트레이드
#        美 對中·캐나다·   → 미국, 중국·캐나다·
#        美 상호관세 발표  → 미국 상호관세 발표
#        美 상호관세 발효일 → 미국 상호관세 발효일
#      MARKET_EVENTS_US +1: 2024-04-25~05-15 "미국 FOMC 매파 보합·4월 고용 부진
#      (금리 경로 불확실성)" — 새 다양화로 잡힌 2024-05-03 매칭 라벨링용.
#
#   3. templates/stocks.html — anomaly.js?v=5 → v=6 캐시 버스트.
#
# [Before / After 매칭]
# Before (다양화 30일):
#   2025-02-06  (관세)
#   2024-12-23  (FOMC 매파)         ← 2025-02-06 와 45일 차이
#   2024-11-15  (트럼프 재선)        ← 2024-12-23 와 38일 차이
# After (다양화 90일):
#   2025-02-06  (관세)
#   2024-05-03  (FOMC 매파·NFP 부진) ← 9개월 차이
#   2022-02-08  (연준 매파·우크라)   ← 2년+ 차이
#
# [trade-off]
# 90일 강제 = 같은 episode 의 연속일/근접일이 모두 묻힘 → 진짜 가까운 매칭 일부
# 손실. 그 대신 서로 다른 macro context 3개를 보여주는 정보량 ↑. 사용자 요청 우선.
#
# [재계산 + DB upsert]
# compute_today_anomaly('us') 즉시 호출 → DB latest row (2026-05-06) 갱신.
# stale 한 2026-05-07 row 는 [120] 운영 노트에서 이미 삭제된 상태라 추가 처치 불필요.
#
# [검증]
# - py_compile feature_anomaly.py OK.
# - 한자 잔존: python ord(ch) 0x4E00~0x9FFF 범위 grep → 0건.
# - 매칭 3건 시간 차이: 2025-02 ↔ 2024-05 (~280일), 2024-05 ↔ 2022-02 (~815일)
#   모두 90일 초과.
...
# ═══════════════════════════════════════════════════════════════════════════════
# [123] 2026-05-08 (UTC) — 이상치 탐지 10년 추이 차트 y축 sqrt → log1p 압축
# ═══════════════════════════════════════════════════════════════════════════════
#
# [개요]
# stocks.html "이상 탐지 탭" 의 10년 D² 시계열 차트 y 변환을 sqrt → log1p
# (= log(1+x)) 으로 교체. 코로나(2020-03) 같은 극단 피크에 평시 변동이 묻혀
# 차별성이 사라지는 시각화 문제 해결.
#
# [왜 — 사용자 피드백]
# 사용자: "10년간 추이 그래프에서 코로나 같은 부분이 너무 솟구쳐 있어서 다른
# 부분들이 차별성이 없어지는데 minmaxscaler를 활용하는건 어때?"
# → MinMaxScaler 는 선형 [0,1] 스케일링이라 이 문제 해결 못함 (코로나가 1.0,
#   나머지는 여전히 0 근처 납작). 비선형 압축 필요.
# 옵션 비교 후 채택:
#   - log1p: 0 입력 안전, 곱셈 거리 = 덧셈 거리 (D² 100→1000 이 1→10 과 같은
#            축 길이 차이) → 평시 변동 자연스럽게 가시화. **채택.**
#   - sqrt (이전): 압축 강도 약함 (코로나 D²=500 → sqrt≈22, 평시 D²=10 → sqrt≈3,
#                 여전히 ~7배 차이로 평시가 납작).
#   - winsorization (99% clip): 극단치 정보 자체가 사라짐 — 코로나 "있긴 있다"
#                               정도로만 보임. 이상 탐지 시각화엔 부적절.
#   - MinMaxScaler: 선형이라 무의미.
#
# [구현 — static/js/anomaly.js renderChart()]
#   1. yTransform 변경
#      Before:  v => Math.sqrt(Math.max(v, 0))
#      After:   v => Math.log1p(Math.max(v, 0))
#      log1p(0) = 0 보장 (axis 하한 안전).
#   2. y축 라벨 역변환
#      Before:  orig = t * t        (sqrt 역함수)
#      After:   orig = Math.expm1(t) (log1p 역함수, = exp(t) - 1)
#      축 눈금에 원래 D² 값을 그대로 표시 (사용자는 변환을 인지 안 해도 됨).
#   3. 주석 업데이트 — "log1p ... 0 입력 안전, 극단치 강하게 눌러 평시 변동
#      가시화" 로 의도 명시.
#
# [수치 비교 — 코로나 피크 대 평시]
#   D² 시계열 가정: 평시 ≈ 10, 코로나 피크 ≈ 500
#   - sqrt 변환:    평시 √10 ≈ 3.16,  피크 √500 ≈ 22.36 → 7.07 배 차이
#   - log1p 변환:   평시 ln(11) ≈ 2.40, 피크 ln(501) ≈ 6.22 → 2.59 배 차이
#   → 평시 영역이 차트 높이의 ~38% 까지 차지 (sqrt 14% → log1p 38%).
#
# [수정 파일]
#   static/js/anomaly.js
#     - renderChart() 의 yTransform: Math.sqrt → Math.log1p
#     - y축 라벨 역변환: t * t → Math.expm1(t)
#     - 주석 1줄 갱신 (sqrt → log1p 의도)
#   templates/stocks.html
#     - anomaly.js?v=6 → v=7 (브라우저 캐시 버스트)
#
# [검증]
#   - 알고리즘·데이터·API 변동 0, 시각화 좌표 변환만 변경.
#   - 사용자 노출 라벨 (y축 눈금 = 원래 D² 값) 의미 동일 — 자문 리스크 가드 무관.
#   - log1p / expm1 둘 다 표준 JS Math (ES2015+) — 브라우저 호환 문제 없음.
#
# [structure.md]
