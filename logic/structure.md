...
마지막 갱신 시점: 2026-05-09 (AI차트 — 30일 예측 모델 폐기 + 유사 과거 패턴 매칭 도입 — 사용자 결정 "예측모델은 사용하지 않을거고, 지금 차트 패턴이 과거에 어떤 구간과 비슷한지 찾고 당시 차트를 오늘 차트 밑에 3개 보여주는 식". AskUserQuestion: window=60일, 유사도=Pearson 상관, 매칭+후속 30일. (1) processor/feature_chart_similarity.py 신규: _is_kr_ticker / _fetch_close (KR pykrx 7년 → yfinance(.KS) 폴백, US yfinance 10년) / _zscore_log_returns (절대가격 무관, 모양 매칭) / find_similar_patterns(ticker, window=60, followup=30, top_k=3). 알고리즘: today 마지막 60일 z-score log-return 과 과거 슬라이드 60일 윈도우 모든 후보 (n - 90일 cutoff = 자기-매칭 차단) 의 Pearson 상관 → 정렬 → DIVERSIFY_GAP_DAYS=90 으로 다양화 (top-K 사이 클러스터 차단) → 매칭 구간 + 후속 30일 가격 OHLC 데이터 + disclaimer 함께 반환. (2) api/routers/chart.py: GET /api/chart/similar?ticker&window=60&top_k=3&followup=30 엔드포인트 신규. /predict 엔드포인트는 기존 KR-차단 그대로 유지 (사용자 요청: 예측모델 불사용). (3) static/js/chart.js: setupPredictButton 본문을 toggleSimilarPatterns 호출로 교체. _similarVisible/_similarData 상태 + _renderSimilarHost / _buildSimilarCardHtml (60+30일 미니 SVG, 매칭 구간 실선·후속 점선·경계 수직선·min/max y라벨, 카드당 유사도 % + 날짜 범위 표시). 옛 setupPredictButton 본문은 setupPredictButton__legacy_unused 로 이름만 바꿔 보존 (호출 X). updatePredictBtnVisibility 의 `&& _currentRegion() !== 'kr'` 제거 — 일봉이면 KR/US 무관 노출 (similarity 는 양쪽 지원). (4) static/js/i18n.js: chart.predictBtn 류 라벨 모두 "유사 과거 패턴 보기" / "유사도" / "후속" / "과거 관찰일 뿐 미래 예측 X" 류로 갱신, chart.similarBtn / chart.similarLoading / chart.similarClose 신규. (5) templates/stocks.html: 버튼 data-i18n 을 chart.predictBtn → chart.similarBtn 으로 + 기본 텍스트 "↗ 유사 과거 패턴 보기 →". chart.js?v=25→v=27, i18n.js?v=9→v=10 캐시 버스트. 자문 가드: 응답 disclaimer + 카드 하단 면책 문구 + 라벨에 "예측" 단어 0. 검증: ast syntax OK, node --check OK.

마지막 갱신 시점: 2026-05-09 (KR AI차트 30일 예측 활성화 ROLLED BACK — 사용자 결정 "예측모델은 사용하지 않을거고" 로 위 항목 (KR pykrx 데이터 폴백 / KR 차단 제거 / 프론트 KR 게이트 제거) 모두 원상 복구. processor/feature4_chart_predict.py 와 api/routers/chart.py 의 /predict 분기 HEAD 상태로 복원. /similar 엔드포인트는 별도 도입 (위 항목).
...
마지막 갱신 시점: 2026-05-09 (부동산 지도 — 행정동 단위 세분화 + 클릭 drill-down + app_cache RLS 해제. (1) 사용자 요청 "각 지역을 좀더 세분화 해봐. 최대한 세분화" + "각 법정동 또는 행정동 경계면을 클릭하고 상세페이지를 조회". 데이터 source 결정 (행정동 GeoJSON + 매핑) → KOSTAT submunicipalities-2018 (33MB) 에서 수도권(서울 11/인천 23/경기 31 prefix) 1138 features 만 필터링 + base_year/etc 슬림화. 자동 매핑(이름+prefix-qualified) 으로 76/77 KOSTAT sgg ↔ 행안부 sgg 5자리 매칭, manual override 7개(부천·의정부·안양·안산·고양·용인·화성) 보강 → 1045/1138 (92%) EMD 가 행안부 sgg 매핑. 결과 frontend-realestate/public/geojson/metro-emd.geojson + static/realestate/geojson/metro-emd.geojson (4.97MB). (2) 백엔드 신규 endpoint api/routers/real_estate.py /emd-overview — region_summary 의 stdg(법정동) 단위 시계열에서 month-over-month change_pct_1m / change_pct_3m 산출, app_cache 적재. compute_emd_overview() 970 stdg rows 산출. (3) 프론트 VWorldMap.tsx — PolygonFeature.subName/noLabel 필드 + onZoomChange/overlayPolygons/onOverlayClick props + overlay useEffect (스트로크+옵션 fill+클릭+라벨 marker). 라벨 줌 임계값 OVERLAY_LABEL_MIN_ZOOM=12. EMD_MIN_ZOOM=11 (Leaflet) 도달 시 emd 폴리곤 lazy load. 흰 stroke 1.0px + fill alpha 0.55, hover 시 0.75. (4) MapScreen.tsx — EmdRow 타입 + _cachedEmdLookup Map<sgg_cd, Map<norm_name, EmdRow>> + normalizeStdgName(s='신흥1동'→'신흥동') + loadEmdLookup() (/api/realestate/emd-overview fetch) + loadEmdPolygons(overviews, lookup) (per-EMD distinct 색상: stdg 매칭 우선, 미매칭 시 parent sgg fallback, 둘 다 없으면 outline 만). handleEmdClick(sggCd, name, stdgCd?) — 기존 sgg 패턴 따라 SelectedRegion build → setSelected → FeatureCard 모달 → 카드 탭 시 navigate('/stdg/{stdgCd}'); topStdgCd null 폴백으로 '/region/{sggCd}' 라우팅 (거래건수 없는 EMD 도 detail 진입 가능). (5) migrations/2026_05_09_disable_rls_app_cache.sql 신규 — ALTER TABLE app_cache DISABLE ROW LEVEL SECURITY. supabase anon key 의 cache upsert RLS 거부 ("42501") 로 매 요청 region_summary 4000+ row live-compute (~12s) 되던 문제 해결. 적용 후 first call 5.5s → second call 1.0s (5× 단축). 검증: SELECT relrowsecurity FROM pg_class WHERE relname='app_cache' → false 확인 + 서버 로그 cache 실패 라인 없어짐. (6) frontend-realestate/src/screens/RankingScreen.tsx 영향 없음. 빌드 산출물 index-BciHYa5u.js / index-lf7p11nn.css. 추후: app_cache 정밀 매칭(면적가중 spatial join), service_role key 분리, 라벨 viewport culling.)

마지막 갱신 시점: 2026-05-09 (랭킹 탭 TOP 5 → TOP 50 확장 — 사용자 요청. api/routers/real_estate.py compute_ranking() 의 trade_recovery[:5] / price_up[:5] → [:50]. RANKING_CACHE_KEY 'ranking' → 'ranking_top50' bump 으로 옛 cache row 자동 무효화 (옛 row 는 orphaned 으로 유지되지만 read 안 됨). 응답 필드명(trade_recovery_top5/price_top5)은 호환 위해 보존 — 내용만 50개. _ranking_price_series 의 mini-chart fetch 도 합집합 sgg ~76개 단일 query 라 비용 증가 미미. frontend-realestate/src/screens/RankingScreen.tsx UI 라벨 "TOP 5" → "TOP 50" 두 곳. 검증: curl /api/realestate/ranking → trade rows=50 / price rows=50, 새 번들 index-Dh8uTAtc.js. uvicorn StatReload stuck 재현 → TaskStop 후 socket cleanup 대기 후 재기동 (burqzmxlt). 추후: production DB 의 옛 'ranking' row 는 그대로 두고, 사용자가 필요 시 DELETE FROM app_cache WHERE cache_key='ranking'.)

마지막 갱신 시점: 2026-05-11 (KR 평소 이탈도 (anomaly_daily) 백필 — 적응형 피처 차원. 사용자 보고 "백필을 하면 되는거 아니야?". scripts/train_kr_hmm 으로 KR HMM 학습 + noise_regime 1403 rows 백필 후, scripts/backfill_anomaly --region kr 실행했으나 0 rows 산출. 원인: KR noise_regime 의 fundamental_gap / vix_term 100% NaN — US 전용 피처. processor/feature_anomaly.py build_feature_panel 에 region 별 모두-NaN 컬럼 자동 제거 로직 추가 — KR 은 8피처 (10 - fundamental_gap - vix_term) 로 D² 계산. _decompose_d2 호출 + feature_vector 빌드 모두 ALL_FEATURES 상수 → panel.columns.tolist() 동적 사용. 재실행 결과 KR anomaly_daily 455 rows 적재 (2024-05-31 ~ 2026-05-08). 현재값 D²=45.3 pct_10y=99.4% (관세 충격 직후). 가장 이상 5일 정상 식별 — 2024-08-05 (391), 2025-04-04 (95.7), 2025-04-08 (95.3). API endpoint 폴백 (live compute 시) 도 동일 8피처로 동작.)

마지막 갱신 시점: 2026-05-10 (KR 마지막 두 라이브-호출 endpoint 도 풀-precompute 로 마이그레이션. 사용자 검토 요청 후 audit 결과 18/20 endpoint 는 이미 precompute 패턴, 남은 2개만 라이브 외부 API/무거운 compute 였음. (1) /api/chart/ohlc — 매 클릭 yfinance/pykrx 외부 호출 1~3s + 인메모리 캐시만 (프로세스 단위 증발). 신규 chart_ohlc_cache 테이블 (PK ticker+interval, payload JSONB) + migrations/2026_05_10_add_chart_ohlc_cache.sql. database/repositories.py upsert_chart_ohlc/fetch_chart_ohlc 신규. api/routers/chart.py _compute_ohlc_payload 추출 + precompute_chart_ohlc(tickers) 신규 + endpoint 3-tier (인메모리 → chart_ohlc_cache DB → 라이브 폴백). 28 ticker × 3 interval = 84 entry. (2) /api/crash-surge/direction — 매 30분 cold call 시 numpy 루프 (전 시계열 × 5/10/20 horizon). _compute_direction_payload 추출 + precompute_direction(region) + endpoint 가 app_cache 우선. 옛 _dir_cache 인메모리 30분 TTL 코드 제거. (3) scheduler/job.py Step 16+17 + scheduler/job_kr.py 15+16) 추가 — US 48+1, KR 36+1 entry. 결과: 풀-precompute 21/21 endpoint 완성. 클라우드 비용/속도 모두 최적 — 모든 사용자 클릭 = DB select 1회. Railway 재배포 후 첫 클릭도 cache hit (DB 영속).)

마지막 갱신 시점: 2026-05-10 (유사 과거 패턴 보기 버튼 — 솔리드 오렌지 강조. 사용자 요청 "주황 컬러 포인트로 강조, 눈에 잘 띄게". .predict-btn-wide 를 아웃라인 → 솔리드 #FF8C00 + 검은 텍스트 + weight 800 + padding 14px + radius 10. hover 는 #FFA533 (lighter), active(:active) 는 #E67E00 (darker). 펼쳐진 상태(.active) 는 강조 톤 다운 — transparent + 오렌지 보더 + 오렌지 텍스트 (다시 누르면 닫힘 의도). 그라디언트/글로우 없음 (AI 디자인 신호 회피). 캐시 버스트 main.css?v=130→131.)

마지막 갱신 시점: 2026-05-10 (AI 차트 탭 후속 — range bar chrome 제거 + 봉 간격 / MA chip 톤 다운. 1차 PR (B 옵션) 후속 정리. (1) .chart-range-section: 카드 chrome (border 1px + background var(--card) + radius 14 + padding 14/16) 모두 제거 → border-top 1px hairline 만 (윗 영역과 시각 분리). 시각 카드 1개 더 감소. (2) .chart-iv-btn (일봉/주봉/월봉): radius 12→6, padding 8→6, border 1.5→1px, transparent background, active 가 #1a1a1a/#FF8C00 + box-shadow → 오렌지 tint background + 색상만 강조. (3) .ma-chip: radius 12→6, padding 4→3px, transparent background. opacity 토글은 유지. 모든 컨트롤 톤이 일관되게 가벼움. 캐시 버스트 main.css?v=129→130.)

마지막 갱신 시점: 2026-05-10 (AI 차트 탭 카드 chrome 정리 — Option B (정보 밀도 유지 + chrome 제거). 사용자 피드백 "AI 차트 탭도 카드섹션 너무 많아 AI 디자인 티남". 시각 블록 9~10개 → 5~6개로 축소. (1) 상단 stats: chart-stats-grid 3개 .chart-stat-card → 1개 .stat-strip + 3 .stat-cell (시황 탭과 동일 패턴 재사용, 1px hairline 분리, hover 정적). chart.js renderTopStats 갱신 + stocks.html 스켈레톤 동기화. (2) 캔들 + 거래량: .card 2개 → 1개 통합 (.chart-combined). 사이에 .chart-vol-divider (1px hairline). 카드 chrome 1개로 축소. (3) predict-btn-wide: 블루-시안 그라디언트 + 큰 box-shadow (AI 디자인 신호) → 아웃라인 보더 + 오렌지 hover (회피). border 1px sep, padding 14→12px, radius 16→10px, font 15→14px. .active 도 그라디언트→오렌지 tinted background. 캐시 버스트 main.css?v=128→129, chart.js?v=34→35.)

마지막 갱신 시점: 2026-05-10 (AI 차트 가격 포맷 — KR 모드 ₩정수, US 모드 $.소수. 사용자 보고 "국내주식 AI차트에서 $15765.00 처럼 표시 — ₩ + 소수 제거". static/js/chart.js _fmtPrice(v, decimals=2) 헬퍼 신규 — _currentRegion()==='kr' 이면 '₩'+Math.round(v).toLocaleString('ko-KR'), 그 외 '$'+v.toFixed(decimals). 적용처: 캔들 툴팁 예측값 + 상하 (708-709), 가격 헤더 chart-price-val (863), 상단 3카드 high/low (880,884), 하단 range 바 minmax (903-904). Y축 fmtYLabel 과 magnitude 카드 SVG y라벨은 통화 prefix 없어 불변. chart.js?v=33→34. KR 069500 예: $15765.00 → ₩15,765. US SPY: $501.20 그대로.)

마지막 갱신 시점: 2026-05-10 (AI 차트 유사 패턴 — 매칭 결과 풀-precompute 로 마이그레이션 (CPU 0). 사용자 지적 "DB 미리적재가 클라우드 비용·속도 면에서 유리한데 왜 슬라이딩으로 했냐". 인정: 기존 raw close 캐시 + 매 클릭 numpy 슬라이딩(~100ms CPU)는 차선. (1) migrations/2026_05_10_add_chart_similarity_cache.sql 신규 — chart_similarity_cache (ticker, mode, payload JSONB, updated_at, PK ticker+mode) + idx + RLS disable. (2) database/repositories.py: upsert_chart_similarity / fetch_chart_similarity 함수 신규 (page_view 섹션 직전). (3) processor/feature_chart_similarity.py: precompute_chart_similarity(tickers) 신규 — 28종 × 2 mode 고정 파라미터 (shape 60/30/3, magnitude 126/126/5) 로 find_*_matches 결과를 JSON 통째 upsert. precompute_chart_closes 는 라이브 폴백용으로 보존. (4) api/routers/chart.py /api/chart/similar — 3-tier: 기본 파라미터일 때만 chart_similarity_cache select 우선, miss/비기본 파라미터는 라이브 numpy 슬라이딩 폴백. _SIMILAR_DEFAULT_PARAMS 상수 신규. (5) scheduler/job.py Step 15 (US 16×2=32) + scheduler/job_kr.py 14) (KR 12×2=24) 추가. 운영: Supabase migration 1회 실행 + 다음 cron 자연 적재. 클라우드 비용: 클릭당 CPU ~100ms → 0ms, 응답 ~300ms → ~200ms, scheduler 측 cron 한 번에 56 entry 산출 (CPU ~5초 1회/일). 폴백 안전 — 마이그레이션 미적용/cron 미실행 환경 동작 무관.)

마지막 갱신 시점: 2026-05-10 (AI 차트 유사 패턴 summary 폴백 — 임계 매치 0건 시 top-K 폴백. 사용자 보고 "국내주식 강도매칭에서 경우의수 텍스트 안 나옴". 진단: KR 069500 today 6개월 누적 +93.45% 라 과거 ±2% 범주 candidate 0건 → summary=None → 프론트가 박스 미렌더. 해결: 두 매칭 함수에 폴백 — magnitude 의 ±2% 범주 0건이면 top-K(=화면 카드 5건)로, shape 의 sim≥0.85 0건이면 top-K(3건)로 summary 산출. summary.fallback_topk True 플래그 추가 (디버그용). 결과: KR 069500 magnitude 검증 — n_matches=5 fallback_topk=True 정상 노출. KR 069500 shape — n_matches=17 정상 임계 통과 (폴백 미발동).)

마지막 갱신 시점: 2026-05-10 (KR/US multiple round-trip endpoint → app_cache 단일 페이로드. 사용자 보고 "국내주식 탭 호출 속도 느림". 진단: 대부분 endpoint 는 이미 precompute + DB cache 패턴이지만 두 곳이 multiple Supabase RTT 로 누적 지연. (1) /api/market-summary/today — 4 fetch 직렬 (fear_greed/index_prices/macro/crash_surge) ~3s. (2) /api/anomaly/history?days=2520 — 페이지네이션 1000-row × 3 = ~6.5s. 해결: 두 endpoint 에 app_cache 우선 + 라이브 폴백 패턴 적용. (a) api/routers/market_summary.py — _compute_market_summary_today_payload() 추출 + precompute_market_summary_today(region) + endpoint 가 fetch_app_cache hit 시 즉시 반환, miss 시 live compute (스케줄러 미실행 환경 안전망). (b) api/routers/anomaly.py — _compute_anomaly_history_payload(days, region) + precompute_anomaly_history(region, days=2520) + endpoint 가 _anomaly_history_cache_key (region+days 별) 우선 select. (c) scheduler/job.py Step 14 추가, scheduler/job_kr.py 13) 추가. 측정: market-summary/today KR 3077→2205ms (-30%), anomaly/history US 6521→2794ms (-57%, 3 RTT → 1 RTT). KR anomaly 는 anomaly_daily 비어 fallback — production 적재 후 동일 효과. 기존 frontend 인-메모리 캐시 없음 — 향후 PR 에 추가 여지.)

마지막 갱신 시점: 2026-05-10 (AI 차트 유사 패턴 — summary 분모 확장 + "수익권" 카운트로 변경. 사용자 요청: 카드는 top-K(5/3) 그대로 보여주되 통계 박스는 더 큰 후보 집합 기준으로 계산. (1) processor/feature_chart_similarity.py 신규 상수 — MAGNITUDE_SUMMARY_TOL_PCT=2.0 (today 누적과 ±2% 이내), SHAPE_SUMMARY_SIM_THRESHOLD=0.85 (Pearson corr). (2) find_magnitude_matches: summary 가 candidates 전체 중 |c_total_pct - today_total_pct| ≤ 2.0 & full followup 인 모든 시점의 후속 분포 — n_matches/post_mean/median/min/max + n_profit_after (post_total_pct > 0 카운트). 기존 n_drawdown_10pct 제거. (3) find_similar_patterns: today_window 에 total_pct 추가, summary 신규 — candidates 중 similarity ≥ 0.85 & full followup 인 모든 시점의 후속 분포 (동일 schema). 응답에 mode='shape' 도 추가. (4) static/js/chart.js _buildMagnitudeSummaryHtml → _buildSummaryHtml 로 통합 (mode 인자 분기 — '비슷한 강도/모양 시점'). 마지막 라인 "후속 ${followup}일 후 수익권: M건/N건" 으로 변경. _renderSimilarHost 가 magnitude/shape 양쪽 모두 summary 박스 렌더. (5) 차트 카드는 매칭률 top-K (magnitude 5건, shape 3건) 그대로 보여줌 — summary 분모만 확장. 캐시 버스트 chart.js?v=32→33. 자문 가드: "수익권" = post>0 사실 진술, "매수/매도/예측" 단어 없음 유지.)

마지막 갱신 시점: 2026-05-10 (홈 + 시황 탭 카드 정리. 사용자 피드백 "카드 섹션이 너무 많다" + Coinbase Wallet 행 리스트 레퍼런스. (1) 홈: 8개 .home-tile 4×2 그리드 → 두 row 컨테이너 (.home-rows) + .home-row (좌측 38×38 오렌지 아이콘 박스 / 가운데 제목 15px + 부제 12px / 우측 chevron). 섹션 분리 — "주요 신호" 5개 (AI차트·시황·펀더멘털·이상 탐지·시장 밸류) + "더 보기" 3개 (거시경제·섹터 밸류·섹터 모멘텀). data-tile 속성 + .home-tile 클래스 그대로 유지 → home.js TILE_MAP 핸들러/KR 가드 무수정. home.css 의 .home-tiles-label/.home-tiles 그리드 룰 → .home-section-label/.home-rows/.home-row/.hr-icon/.hr-text/.hr-title/.hr-sub/.hr-chev 로 교체. (2) 시황 탭: VIX/VOL/P/C 3개 .ind-card → 1 컨테이너 .stat-strip + 3 .stat-cell (border-left 1px hairline 분리). main.css 의 .ind-grid/.ind-card/.ind-label/.ind-value/.ind-status 룰 + 호버/액티브 애니메이션 룰 모두 .stat-strip/.stat-cell/.stat-label/.stat-value/.stat-status 로 교체 (정적, 애니메이션 없음). main.js _applyRegionMarketLabels 셀렉터도 동기화 (.stat-strip/.stat-cell/.stat-label). KR 모드 .kr-2col 동작 보존. 캐시 버스트 main.css?v=127→128, main.js?v=136→137, home.css?v=16→17.)

마지막 갱신 시점: 2026-05-10 (헤더 ⚙️ 이모지 → 미니멀 SVG 기어 아이콘 + AI 차트 유사 패턴 로딩 ⌛/⏳ 이모지 제거. templates/stocks.html #btn-edit-holdings 의 ⚙️ 텍스트를 inline svg(circle r=3 + 8-tooth gear path, stroke 1.7px, currentColor)로 교체. main.css .settings-btn font-size:20px 제거 + display:inline-flex 정렬 + color:var(--text-sub) 로 stroke 자동. static/js/chart.js + i18n.js (ko/en) 의 chart.similarLoading/predictLoading 에서 ⌛ 제거 — 텍스트만. 캐시 버스트: stocks.html main.css?v=125→126, i18n.js?v=10→11, chart.js?v=31→32.)

마지막 갱신 시점: 2026-05-10 (탭 진입 시 헤더 좌측 padding 미적용 버그. 사용자 스샷 — "← 홈" 버튼이 5월 10일 / Passive 로고와 겹침. 원인: home.css 의 `body.with-back .app-header > div:first-child` 셀렉터가 햄버거 button 이 실제 first child 라서 매치 안 됨 (div 가 아니라). 수정: `div:first-of-type` 으로 변경 + padding-left 64px→72px (back 버튼 폭 + 안전 여백). home.css?v=14→15.)

마지막 갱신 시점: 2026-05-09 (헤더 햄버거 메뉴 — 탭 진입 시 숨김. 사용자 보고 "탭에 들어가면 ← 홈 버튼이랑 좌측 상단에서 겹친다". home.js showTab/showSectorTab 이 이미 body.with-back 클래스를 토글하므로 CSS 한 줄 추가 — static/css/main.css `body.with-back .menu-btn { display: none; }`. 햄버거는 홈 화면에서만 표시. templates/stocks.html + templates/about.html main.css?v=124→v=125 캐시 버스트.)

마지막 갱신 시점: 2026-05-09 (AI 차트 유사 패턴 — Supabase 적재 + in-memory LRU 도입 (옵션 1+2). 사용자 보고 "강도 매칭 로딩 너무 느림". 원인: _fetch_close 가 매 클릭마다 yfinance/pykrx 외부 API 호출 (1~3s). 해결: 3-tier fetch chain. (1) migrations/2026_05_09_add_chart_close_cache.sql 신규 — chart_close_cache 테이블 (ticker, date, close, updated_at, PK ticker+date) + idx_ticker_date 인덱스 + RLS disable. (2) database/repositories.py: upsert_chart_closes(ticker, dated_closes, chunk=500) + fetch_chart_closes(ticker, days=2520) 두 함수 신규. (3) processor/feature_chart_similarity.py: _close_cache 모듈 dict + _close_cache_lock + _CLOSE_CACHE_TTL_SEC=6h. _fetch_close 분해 — _fetch_close_external (기존 yfinance/pykrx 본체), _fetch_close_from_db (Supabase select 후 pd.Series 재구성), _fetch_close (3-tier wrapper: in-memory hit → DB hit → external 폴백, 어느 단계 성공이든 메모리 캐시 채움). precompute_chart_closes(tickers) 신규 — scheduler 용 list 의 각 ticker 를 외부 fetch + DB upsert + 메모리 캐시. (4) scheduler/job.py Step 13 추가 (US 16개), scheduler/job_kr.py 12) 추가 (KR 12개). 결과: 로컬 측정 SPY 첫 호출 4.0s → 캐시 hit 0.26s (15× 빠름), KR 069500 2.0s → 0.22s (9× 빠름). scheduler 1회 돌면 첫 호출도 ~200ms. 운영: Supabase 에서 마이그레이션 SQL 실행 + 다음 cron 자연 적재. 미적재 ticker 는 외부 API 폴백 자동 (회귀 X). + 라벨 색상: 강도 매칭 카드 헤더 "#1 매칭 +X%" → "#1 [회색]지난 6개월 수익률 [녹/적]+X%" (양수 var(--green) 음수 var(--red)). chart.js?v=30→31. + 세그먼트 가운데 정렬 (display flex + margin auto). main.css?v=123→124.

마지막 갱신 시점: 2026-05-09 (KR 시장 밸류 5-component composite + CLAUDE.md 보고 형식 룰 업데이트. 사용자 보고 "올해 코스피 50%+ 급등(2500→8000)인데 시장 밸류가 저평가로 측정됨". 원인: 기존 composite = 0.4·z_ERP + 0.3·z_VKOSPI + 0.3·z_DD60 — 모두 평균회귀형 단기 sentiment. 절대 가격 수준(PER 레벨, 추세 위치) 미반영이라 급등 후 평온 시 셋 다 "저평가" 기여. 해결: collector/valuation_signal_kr.py 에 두 baseline 함수 신규 — _compute_kospi_per_baseline_15y (pykrx 15Y daily PER mean/std), _compute_kospi_trend_baseline_5y (close/200d MA - 1 의 5Y 분포). 가중치 재정의 W_PER 0.35, W_TREND 0.25, W_ERP 0.15, W_VIX 0.10, W_DD 0.15 (구조 valuation 0.6 + 단기 sentiment 0.4). compute_composite_z 시그니처 +kospi_per/+price_vs_ma200, 둘 다 부호 반전(높은값=비쌈=음수 기여). fetch_valuation_signal_today_kr 가 close 1Y 마지막 200일 MA 로 price_vs_ma200 계산해 인자 전달. fetch_valuation_signal_history_kr (backfill) 도 pykrx fundamental 일별 PER + close/MA200 ratio 시리즈 추가 결합 후 5-component 호출. get_kr_baselines 가 옛 스키마 캐시(per_15y 키 없음) 자동 무효화. 옛 models/valuation_baselines_kr.json 삭제 후 재생성 — 새 스키마(5 keys + new weights). 수치 시뮬: PER 27 + 추세 +30% + ERP 0.002 + VKOSPI 20 + DD60 -0.02 → z_comp=-2.04 → "명확한 고평가". 로컬 pykrx/FDR 미설치라 실제 baseline n=0 fallback 으로 머무르지만 production 첫 cron 에서 실측 baseline 자동 재계산. CLAUDE.md ## 구현 완료 보고 형식 [변경 위치] 룰 업데이트 — "변경 후 코드 스니펫만" → "변경 후 위치·로직 설명만 (코드 스니펫·Before/After diff 모두 금지)". 자연어 묘사로 통일.)

마지막 갱신 시점: 2026-05-09 (KR 시장 밸류 5-comp 후속 — DB 컬럼 + frontend 표시. 사용자 보고 "여전히 다소 저평가로 나옴" 진단: 백엔드 5-comp composite 로직은 적용됐으나 (1) DB 의 valuation_signal 테이블에 z_per/z_trend/price_vs_ma200 컬럼 부재 → 새 component 값이 저장 안 됨 (2) frontend 의 공식 설명 + 분해 표시가 옛 3-component 하드코딩. 해결: (1) migrations/2026_05_09_add_valuation_per_trend_columns.sql 신규 — ALTER TABLE valuation_signal ADD COLUMN IF NOT EXISTS z_per/z_trend/price_vs_ma200 DOUBLE PRECISION. (2) api/routers/macro.py build_baseline_snapshot — per_15y/trend 키도 응답에 포함 (backward-compat: 키 없으면 빈 dict). (3) static/js/home.js — has5Comp 디텍션(W.per_15y && W.trend && bs.per_15y && bs.trend) 로 5-comp 활성 여부 판단. 활성 시 공식 텍스트 "PER 레벨(35%) + 추세 위치(25%) + 주식 매력도(15%) + 공포(10%) + 하락충격(15%)" 로 변경, 분해 보기에 PER/추세 raw 행 + z_per/z_trend σ 행 추가 (각각 15Y baseline mean 표기, 추세는 5Y 평균 표기). 미활성(US 또는 KR-legacy) 시 옛 3-comp 표기 유지. templates/stocks.html home.js?v=44→v=45 캐시 버스트. 검증: py_compile + node --check OK. 사용자 적용: Supabase SQL editor 에서 migration SQL 실행 → KR 다음 cron(KST 16:00) 시 새 컬럼 적재 → 화면 5-comp 표시 + "다소 저평가" 라벨이 새 z_comp 기준으로 갱신.)

마지막 갱신 시점: 2026-05-10 (US 시장 밸류 6-component composite 도입. 사용자 결정 "6-component으로 진행해" — 웹서치 결과(Buffett indicator 230% / Shiller CAPE 40+ / Swinkels-Umlauft 2022 학술 backing) 종합. 기존 0.4·z_ERP + 0.3·z_VIX + 0.3·z_DD60 (단기 sentiment 만) → 구조 valuation 두 축(CAPE·Buffett) + 추세 + 단기 sentiment 셋 의 6-comp 으로 확장. 가중치 재정의 W_CAPE 0.25, W_BUFFETT 0.25, W_TREND 0.20, W_ERP 0.10, W_VIX 0.10, W_DD 0.10 (구조 0.50 + 추세 0.20 + 단기 0.30). collector/valuation_signal.py 전면 재작성 — _load_shiller_df (CAPE 열까지 정규화), _compute_cape_baseline_15y (Shiller 15Y monthly), _compute_buffett_baseline_15y (FRED public CSV 직접 다운로드 — pandas_datareader 없이 fredgraph.csv?id=WILL5000PRFC + GDP, GDP forward-fill 후 비율), _compute_trend_baseline_5y (^GSPC close/MA200 5Y), 기존 ERP/VIX/DD baseline 유지. compute_composite_z 시그니처 +cape +buffett_ratio +price_vs_ma200, 셋 모두 부호 반전(높은값=비쌈=음수 기여), 결손 시 z=0 fallback. fetch_valuation_signal_today: 1y ^GSPC fetch 로 MA200 warmup + Shiller CAPE 마지막 값 + FRED Buffett 마지막 값. backfill_valuation_signal: 200d warmup 위해 years_for_close=max(2,...), 일별 CAPE forward-fill (월별→일별), 일별 Buffett (WILL5000PRFC 일별 / GDP ffill), 5d rolling 평활 (스파이크 제거). get_baselines: 6 키 (erp/vix/dd/cape_15y/buffett_15y/trend) + weights, 옛 3-key cache 자동 무효화. migrations/2026_05_10_add_valuation_us_6comp_columns.sql 신규 — ALTER TABLE valuation_signal ADD z_cape/z_buffett/cape/buffett_ratio DOUBLE PRECISION. api/routers/macro.py build_baseline_snapshot 가 cape_15y/buffett_15y 키도 응답 포함, baseline_snapshot 강제 재구축을 KR→region 무관으로 확장 (옛 3-comp weights 박힌 row 자동 무시). build_valuation_interpretation 의 contribs 가 baselines.weights 기반 동적 산출, CAPE/Buffett/추세 점수 분기 cause 추가. static/js/home.js — has6Comp 디텍션(W.cape && W.buffett && W.trend && bs.cape_15y && bs.buffett_15y && bs.trend), 6-comp 활성 시 공식 "Shiller CAPE(25%) + Buffett 비율(25%) + 추세 위치(20%) + 주식 매력도(10%) + 공포(10%) + 하락충격(10%)", 분해에 CAPE/Buffett/추세 raw 행 + z_cape/z_buffett/z_trend σ 행 추가, KR 5-comp 분기는 그대로. templates/stocks.html home.js?v=46→v=47. 검증: ast.parse OK. 사용자 적용: Supabase 에서 migration 실행 → US 다음 cron 실행 시 6-comp 적재. 단계적 동작: 첫 cron 직후 baseline n>0 → 새 z_cape/z_buffett/z_trend 활성, 그 전엔 fallback (cape mean=28/std=6, buffett mean=1.5/std=0.45, trend mean=0.04/std=0.10) 으로 합리적 기본값.)

마지막 갱신 시점: 2026-05-10 (시장 밸류 추이 그래프 60일 → 10년 확장 + Buffett FRED-차단환경 fallback. 사용자 요청 "10년으로 가능?". 옵션 4안 제시(10y daily+downsample / 10y daily / 10y weekly / 5y daily) 중 사용자 선택 1번. (1) collector/valuation_signal.py — _wb_gdp_usa (World Bank API USA NY.GDP.MKTP.CD 연간 시리즈 → $B 단위), _yf_wilshire_history (yfinance ^W5000 daily), _buffett_series (1순위 FRED WILL5000PRFC/GDP, 2순위 yfinance ^W5000 / WB GDP) 신규. _compute_buffett_baseline_15y, _fetch_buffett_today, backfill_valuation_signal 가 _buffett_series 단일 helper 통일. backfill 의 DataFrame 변환 시 None→NaN 자동 변환되는 문제 → math.isnan 가드로 NaN→None 변환 (Supabase JSON serialize 시 nan 거부 회피). (2) database/repositories.py fetch_valuation_signal_history — Supabase anon key 1000행 호출 제한 → .range(start,end) 페이지네이션, days>1000 이면 1000행씩 chunk fetch. (3) api/routers/macro.py — endpoint 에 days param (10~5000, 기본 90) 추가, days>500 일 때 _weekly_downsample (5거래일 평균, numeric 평균/label·region 마지막) 자동 적용. cache_key 가 region+days 조합으로 분리. (4) static/js/home.js — fetch URL 에 ?days=2520 추가 (withRegion 포함), renderCompositeHistory 의 "3개월 전" 라벨을 _xLeftLabel 가변 함수로 교체 (history[0].date 와 오늘 차이 → "10년 전"/"5년 전"/"1년 전"/"N개월 전"/"N일 전"). buffett_ratio==null 이면 "데이터 없음" 표시. (5) templates/stocks.html — "3개월 추이" → "10년 추이". home.js?v=47→48. (6) US 10y (2520행) + KR 10y (2520행) backfill 실측 적재 완료. US: yfinance ^W5000+WB GDP 로 buffett 2509/2520 채움, today z_comp=-1.17 명확한 고평가, history[0]=2016-05-11 z=+1.05 다소 저평가, weekly=504 pts. KR: pykrx/ECOS 차단환경이라 DART KOSPI200 시총가중 PER 평탄+10Y fallback 3.5% 평탄으로 적재(production cron 에서 자연 갱신), today z_comp=-2.02 명확한 고평가, weekly=504 pts. (7) 비용: DB 5040행 추가=2.5MB(Supabase 500MB tier 0.5%), payload weekly 후 ~85KB+gzip ~50KB, 첫 호출 5s(페이지네이션+downsample), 이후 24h 캐시 hit ms 응답.)

마지막 갱신 시점: 2026-05-11 (펀더멘털 탭 — 에디토리얼 미니멀 리디자인 (AI 자동생성 디자인 인상 제거). 사용자 요청 "AI 디자인 티 벗는다" + 추천안 (토스 풍 미니멀 베이스 + Koyfin 풍 overline·모노스페이스 한 스푼) 채택. (1) templates/stocks.html 펀더멘털 탭 AI 해설 카드 — 보라색 인라인 배지 (`background:rgba(139,92,246,0.15);color:#8B5CF6`, "AI") 제거, card-label 텍스트 "AI 해설" → "분석 노트" (data-i18n="ai.explainLabel" 유지), 본문 아래 .ai-explain-footer "자동 생성된 해설 · 참고용" 작게 추가. 다른 탭(섹터·신호·차트·섹터 밸류)의 AI 해설 카드는 변경 없음(보라 배지 유지). (2) static/css/main.css 끝에 `#tab-fundamental` 스코프 오버라이드 섹션 신규 — .card 글래스 chrome 제거(backdrop-filter none, solid bg, radius 20→12, padding 18/16→16/14, box-shadow none), .section-label/.card-label 11~12px uppercase + letter-spacing overline (Koyfin 톤), .badge 사각 모서리·glow shadow 제거(important), .nr-name tabular-nums + 18px, .nr-icon-box 40→36px·radius 12→8px, .nr-gap-track height 6→4·shadow 제거, .nr-gap-dot 14→12px·dotGlow 애니 제거, .ai-explain-card 보라 top border 제거 + 좌측 2px sub2 quote line(::before), .ai-explain-card .badge display none(인라인 보라 배지 잔재 가드), .ai-explain-footer 10.5px sub2 hairline-top, .disclaimer 11px sub2, .card-tappable::after accent → sub 중성 톤. 다크모드 분기: card bg #131316→#0B0B0E (Bloomberg 톤 강조), quote line 알파 0.18. 다른 탭(섹터/신호/차트/섹터 밸류) 미영향 — 모든 룰 #tab-fundamental 스코프. 검증: 단순 CSS append + HTML 1블록 교체, JS·i18n·라우터 무수정. main.css?v=131→132, about.html 동기.)

마지막 갱신 시점: 2026-05-11 (펀더멘털 탭 후속 — 카드 섹션 통합 (3 → 1). 사용자 피드백 "카드 섹션이 너무 많지 않냐" — 시황(stat-strip)·차트(chart-combined) 정리 패턴이 펀더멘털엔 안 들어갔다는 지적 동의. (1) templates/stocks.html — 기존 .card 두 개(Noise vs Signal + #nr-chart) 를 단일 `.card.fundamental-combined` 로 병합. 내부 구조: `<section.fc-regime>` (regime card-head + #regime-card) → `<div.fc-divider>` (1px hairline, 카드 좌우 padding 가로지름 -16px margin) → `<section.fc-chart>` (.fc-section-label overline + #nr-chart). 분석 노트는 `class="card ai-explain-card"` → `class="ai-explain-plain"` 로 카드 chrome 완전 제거 — 풀폭 텍스트 + 좌측 2px quote line + 11px overline + 푸터 hairline. (2) static/js/main.js line 803 — `container.closest('.card')` → `container.closest('.fc-regime') || container.closest('.card')`. 통합 카드의 클릭 영역을 regime 영역으로만 한정 → 차트 클릭 시 noise detail 페이지 안 열리는 문제 회피. 폴백 chain 으로 다른 컨텍스트 회귀 없음. (3) static/css/main.css — 옛 `.ai-explain-card` 룰 블록 삭제, 신규 `.fundamental-combined`/.fc-regime/.fc-divider/.fc-chart/.fc-section-label/.ai-explain-plain/.ai-explain-overline 룰 추가. fc-divider margin -16px 좌우 → 카드 padding 을 가로지르는 풀폭 hairline (chart 탭 .chart-vol-divider 유사). ai-explain-plain 은 padding-left:16px + ::before quote line, 카드 border 없음. 검증: HTML 1블록 교체, JS 1줄, CSS 룰셋 교체 — i18n·백엔드·라우터 무수정. 데이터 흐름 무변(같은 #regime-card / #nr-chart / #ai-explain-fundamental-text 타겟). 캐시 버스트 main.css?v=132→133, main.js?v=137→138, about.html 동기.)

마지막 갱신 시점: 2026-05-11 (AI 차트 탭 — 시각 블록 추가 정리 + wide CTA 인라인 토글화. 사용자 피드백 "AI 차트 탭도 카드 섹션이 많은 것 같다" — chart-combined 1카드 + stat-strip 1카드 = 카드 chrome 2개, 컨트롤 4단(매칭 모드/CTA/봉간격/MA 범례) 분산이 잔재. 추천안: (a) wide 풀폭 주황 CTA "↗ 유사 과거 패턴 보기 →" → 인라인 텍스트 토글 (b) MA 범례 차트 카드 안으로 흡수 (c) stat-strip chrome 제거 — 모두 채택. (1) templates/stocks.html 차트 탭 — 기존 [매칭 모드] [wide CTA 풀폭] [봉간격] [MA 범례] [차트 카드] 5블록 → [봉간격] [매칭 컨트롤 한 줄(좌:모드 세그·우:인라인 토글)] [차트 카드(MA 범례 흡수)] 3블록. .chart-similar-controls > .chart-controls-row 신규 (flex space-between), predict-toggle-btn class predict-btn-wide → predict-toggle-link, 내부 텍스트는 i18n 키 그대로. ma-legend 를 .card.chart-combined 첫 자식으로 이동(JS 는 ID 만 참조 → 위치 무관). predict-toggle-btn?.parentElement = 외부 .fade-target 그대로 → chart.js 의 similar-cards-host append 위치 보존(회귀 X). (2) static/js/i18n.js — 'chart.similarBtn'/'chart.predictBtn' "↗ 유사 과거 패턴 보기 →" → "유사 패턴 보기 ▾" (KR), "Similar patterns ▾" (EN). 'chart.similarClose' 에 "▴" 접미. (3) static/css/main.css 끝에 `#tab-chart` 스코프 신규 — stat-strip 글래스/border/radius 제거 + border-top/bottom 1px hairline 으로 셀 strip 화 (.stat-cell padding 14→10), .chart-controls-row flex space-between, .similar-mode-seg margin 0, .predict-toggle-link 신규 룰 (small inline button, 6/10 padding, sep border, transparent bg, .active 시 오렌지 outline tinted bg, hover 시 darken — 풀폭/대형 강조 모두 제거), .chart-combined > .ma-legend 첫 줄 흡수 (margin 0/8 padding 4/8/6, 좌측 정렬, hairline-bottom, :empty 시 hide). 검증: HTML 컨트롤 1블록 재구성, CSS append, i18n.js 텍스트 토큰만. JS 무수정 — predict-toggle-btn ID/parentElement 보존, ma-legend ID 보존, similar-cards-host append 위치 보존. 캐시 버스트 main.css?v=133→134, i18n.js?v=11→12, about.html 동기.)

마지막 갱신 시점: 2026-05-11 (KR 평소 이탈도 — KR 시장 이벤트 사전 + 알고리즘 정합성 + 데이터 폴백 다층화. 사용자 두 보고: ① "k-NN 카드에 모두 '기록된 시장 이벤트 없음'" ② "1년에 KOSPI 3배 상승했는데 D² 가 이거밖에 안 됨". 해결: (1) static/js/anomaly.js MARKET_EVENTS_KR 31개 신규(2014 세월호 ~ 2025 美 상호관세) + _isKrMode 분기 findEvent. (2) processor/feature_anomaly.py build_feature_panel — region != 'us' 면 yfinance 외부 extras (^TNX/^IRX/^VIX) 호출 skip → KR 이 US 매크로 잡음 안 받음. (3) collector/noise_regime_data_kr.py fetch_kr_10y_monthly — ECOS/FDR 둘 다 실패 시 FRED CSV 직접 fetch (IRLTLT01KRM156N, OECD KR 10Y monthly 1990s~) → 83월 확보 → erp_zscore 정상 산출. macro_raw 4차 폴백도 보존. (4) erp_zscore rolling min_periods 적응형 — min(24, max(3, n//2)) 로 짧은 KR 10Y 도 산출 가능. (5) compute_monthly_features_kr — 100% NaN 컬럼 자동 제거 + 유효 행 수 < 12 컬럼 제거(긴 시계열 다른 피처 보존). (6) processor/feature1_regime.py train_hmm — 누락 피처 무시(교집합으로 학습, 하한 3 피처). (7) HMM transmat/startprob Laplace smoothing (eps=1e-3) — 짧은 학습 데이터에서 row sum=0 인 state 발생 시 predict ValueError 회피. 결과: KR HMM 35월 × 8피처 학습 + 1403 noise_regime backfill + 478 anomaly_daily backfill. 오늘(2026-05-08) D²=74.3 pct_10y=100% (10년 중 가장 이상한 날 — 사용자 직감 정확). top contributors: amihud(54.4)/erp_zscore(18.4, 2.48σ)/hy_spread(4.4) — KR 자체 신호 dominant.)

마지막 갱신 시점: 2026-05-11 (AI 차트 탭 컨트롤 시각 조정 — 가운데 정렬 + 토글 주황 포인트. 사용자 피드백 + 스크린샷 "이 부분 가운데 정렬, 유사 패턴 버튼 주황 포인트로 눈에 띄게". 진단: .chart-controls-row 가 justify-content space-between + flex-wrap 이라 모바일 좁은 폭에서 모드 세그/토글이 두 줄로 wrap 될 때 좌·우 정렬 따로 → 어색. predict-toggle-link 가 기본 sep border/transparent bg 라 너무 톤 다운돼 primary action 으로 안 보임. (1) static/css/main.css #tab-chart .chart-controls-row — justify-content space-between → center, gap 12→10. .predict-legend 도 justify-content center 추가. .similar-mode-seg flex 0 1 auto → 0 0 auto (자체 폭만). (2) .predict-toggle-link 기본 상태 톤 업 — background transparent → rgba(255,140,0,0.08), border var(--sep) → #FF8C00 솔리드, color var(--text) → #FF8C00, font-weight 600→700, padding 6/10→7/14. hover 시 background 0.16 + border #FFA533. .active 시 솔리드 채움 background #FF8C00 + color #000 (펼친 상태 강조). 다크모드 hover 룰 제거(불필요). 검증: CSS 룰 부분 교체. HTML/JS 무수정. 인라인 토글이지만 주황 outline + 4% tint bg → 첫 진입자가 명확히 인식. 캐시 버스트 main.css?v=134→135, about.html 동기.)

마지막 갱신 시점: 2026-05-11 (홈 화면 — 2열 그리드 카드형으로 환원. 사용자 스크린샷 첨부 + "이 형태로 되돌리려고 한다". 진단: 기존 home-rows 행 리스트(Coinbase Watchlist 패턴) 가 압축적이지만 사용자가 첨부한 형태는 8 타일을 2열 카드 그리드로 균질 배치한 옛 home-tile 톤. home.css 의 .home-tile/.ht-icon/.ht-title/.ht-sub/.ht-badge 룰은 보존돼 있음(.home-row 룰만 위에 추가돼 있었음). (1) templates/stocks.html — 기존 [주요 신호] .home-rows + [더 보기] .home-rows 두 섹션 (5+3 행 리스트) → 단일 [탐색] .home-tiles 그리드 (8 카드). 카드 순서 스크린샷 매핑: 1.AI차트 2.시황 3.펀더멘털 4.평소 이탈도 5.거시경제 6.섹터 밸류에이션(NEW) 7.섹터 모멘텀(NEW) 8.시장 밸류(NEW). data-tile 값(ai-chart/market/fundamental/signal/macro/sector-val/sector-mom/market-valuation) + i18n key(tile.anomaly/tile.anomalySub) 보존 → home.js TILE_MAP 핸들러/KR 가드 무수정 회귀 X. 각 카드: .ht-icon (오렌지 38×38 라운드 박스) + .ht-title 14px 굵게 + .ht-sub 11px sub2 + NEW 카드는 .ht-badge 우상단 오렌지 pill. hr-icon/hr-text/hr-title/hr-sub/hr-chev 행 마크업 모두 삭제. (2) static/css/home.css — .home-tiles 신규 룰 추가 (grid 2 columns, gap 10, 좁은 폭 ≤360px gap 8). .home-rows/.home-row 룰은 보존(다른 곳 사용 가능성). 검증: HTML 1섹션 교체. CSS append. JS 무수정. 캐시 버스트 home.css?v=17→18, 다른 캐시 미변경. about.html 영향 없음 (홈 화면은 stocks.html 만).)

마지막 갱신 시점: 2026-05-16 (US 거시경제 회복→둔화 보수화 — FRED leading 3종 추가. 사용자 보고 "현재 회복국면 분류 너무 낙관적, 보수화하고 싶다". 후보 제시 후 사용자 결정 "추천 최우선 3개 추가". (1) collector/sector_macro.py FRED_SERIES 에 SAHMREALTIME→sahm (Sahm Rule 실시간 침체신호, >=0.5 시 침체 진입) / DRTSCILM→loan_tighten (은행 대출기준 강화 %, 분기 → 월별 ffill(3), 신용경색 leading) / CFNAI→cfnai (시카고 연준 국가활동지수, 85개 지표 종합 coincident) 3 항목 추가. df_sahm/df_loan/df_cfnai 전처리(분기 resample('MS').last().ffill(3) for loan, 월별 그대로 for sahm/cfnai) 후 macro outer join + _obs_sources 등록. to_sector_macro_records 의 record dict 에 'unrate','hy_oas','vix','sahm','loan_tighten','cfnai' 6종 NaN-safe loop append (NaN 행은 컬럼 누락 → Supabase 가 NULL 로 저장). (2) processor/feature2_sector_cycle.py FEATURE_COLS_US 에 sahm/loan_tighten/cfnai 3개 append (총 16피처). non_empty subset + ffill(limit=3) 로직이 이미 sparse 시리즈 graceful 처리 — 초기 데이터 없는 행은 dropna 로 자동 제외. HMM 6-state 학습 데이터에 leading indicators 가 추가되면 score=pmi+0.5·pmi_chg3m 외 means_ 분포에 신규 피처 반영 → rank 4 (회복) state 가 sahm 상승·loan_tighten 강화·cfnai 음수 시 rank 2~3 (둔화) 으로 자연 강등 효과. (3) migrations/2026_05_16_add_sahm_loan_cfnai.sql 신규 — sector_macro_raw 에 unrate/hy_oas/vix (2026-05-13 누락분) + sahm/loan_tighten/cfnai 6 컬럼 ADD COLUMN IF NOT EXISTS. supabase_tables.sql 동일 6 컬럼 추가. (4) database/repositories.py fetch_sector_macro_history SELECT 컬럼 리스트에 cpi_yoy/ism_pmi/unrate/hy_oas/vix/sahm/loan_tighten/cfnai 8 컬럼 추가 (옛 누락분 보강). (5) 검증: 3 파일 ast.parse OK. 사용자 액션: Supabase SQL editor 에서 마이그 실행 → 다음 US cron(KST 09:00) 자연 적재 → HMM 재학습 권장(scripts/train_us_sector_cycle 또는 1회 수동) 후 회복 판정 분포 변동 관측. 보수화 효과 부족 시 후속 옵션 (사후 강등 필터·RANK_TO_PHASE 변경·점수식 가중치) 검토.)

마지막 갱신 시점: 2026-05-16 (KR Noise HMM 재학습 + 펀더멘털 탭 다중 개편. (1) 그래프 기간 토글 (10년/1년) — anomaly D² / market-valuation Composite Z / fundamental-gap 3 차트에 attachPeriodToggle 헬퍼 (main.js) + .period-toggle CSS (main.css). 사용자 요청 "10년/1년 만, 1M 제거 + 한국어 라벨". (2) KR fundamental_gap 9년 백필 — collector/noise_regime_data_kr.fetch_kospi_shiller_like(years=10) 결과로 2017-04 부터 110 월 row noise_regime KR upsert. 옛 2021+ → 2017+ 분포로 확장. (3) realized_vol 제거 — 사용자 결정: 실현 변동성은 펀더멘털-주가 괴리와 직접 상관 X. FEATURE_NAMES_KR 에서 realized_vol → per_zscore_5y 로 교체. (4) per_zscore_5y 추가 — KOSPI PER 5년 rolling z-score (절대 PER 수준, fundamental_gap 의 변화율 보완). compute_monthly_features_kr 의 P/E 시계열 기반 산출. backfill_noise_regime 의 full_values dict 에 per_zscore_5y forward-fill 추가. NOISE_WEIGHTS_KR: per_zscore_5y weight 0.5 (abs). (5) KR HMM 재학습 — scripts/train_kr_hmm --years 10 --backfill 3650 실행. models/noise_hmm_kr.pkl 7피처 4-state 재학습. 1408 row DB 재적재. (6) i18n.js feat.per_zscore_5y / featDesc.per_zscore_5y 라벨 추가 (한국어 + 영문). main.js NR_FEATURE_KEYS 에 per_zscore_5y 추가. (7) 캐시 버스트 main.css?v=140, main.js?v=162, home.js?v=53, anomaly.js?v=22, i18n.js?v=13. 검증: KR latest noise_score -1.67 (옛 -19.98 → realized_vol 제거 + per_zscore_5y 추가 후 정상화). 화면 "펀더멘털 반영도 구성" 막대에 per_zscore_5y "PER Z점수 (5년)" 한국어 노출.)

마지막 갱신 시점: 2026-05-13 (홈 화면 "오늘의 종합 판단" 카드 — 1~2문장 brief 로 압축. 사용자 요청: "RSI 과매수/과매도 + 지수 일일 수익률 + 펀더멘털-주가 갭 차이 상위 N% 위치, 3가지만". (1) api/routers/market_summary.py — get_home_headline fallback 경로 신규 _build_home_brief(lang, region) 함수로 교체. RSI 임계(≥70 과매수/≥60 근접/≤30 과매도/≤40 근접/그 외 중립) + 지수 일일 수익률 (region US: macro.sp500_return decimal ×100 변환, KR: 이미 percent 그대로) + 펀더멘털-주가 갭 (regime.get_fundamental_gap top_pct 사용, 펀더멘털 탭과 동일 data source). 출력 형식: "{idx} 일일 수익률 {±X.XX}%, RSI {Y.Y} {라벨}. 펀더멘털-주가 갭 차이 상위 {Z}% 위치." (2) fetch_macro_latest2 import 추가 — macro_raw 최근 5건 폴백으로 sp500_rsi/return null 시 과거 행에서 보완 (주말/장후/지표 시차 대비). 검증: US=`S&P500 일일 수익률 -0.16%, RSI 74.5 과매수 영역. 펀더멘털-주가 갭 차이 상위 12.0% 위치.`, KR=`KOSPI 일일 수익률 +2.76%, RSI 86.5 과매수 영역. 펀더멘털-주가 갭 차이 상위 4.0% 위치.` 이전 변경 → "주가-실적 일치/불일치" 라벨에서 이번 변경으로 "갭 차이 상위 N% 위치" 로 재교체 (펀더멘털 탭 데이터와 동일 source 통일).)

마지막 갱신 시점: 2026-05-13 (AI 해설 전면 재설계 — 시황탭 스토리형 1단락 + 5탭 단계별 [결과 요인] 단일 블록 + "이성/감정" 단어 폐기. 사용자 요청: "스토리형 — 흐름 한 단락", "이성 점수 단어 어디에도 사용 X", "왜 이런 결과 부분에 분해 데이터 풀이", "데이터 요약·참고 블록 제거", "거시경제에서 매크로 수치가 나온 실물 원인 풀이", "왜 이런 결과 → 결과 요인 라벨 변경". (1) api/routers/market_summary.py — _fallback_ai_summary KO 분기 4줄 구조 → 스토리형 1단락(심리·이탈도·펀더멘털 반영도·경기국면 4문장 마침표 연결). _fallback_ai_explain 5탭(fundamental/signal/sector/sector-val/sector-mom) KO 분기 모두 옛 [오늘 한눈에]+[왜 이 결과]+[참고] 3블록 → [결과 요인] 단일 블록 + ①입력 ②계산/분류 ③결과 ④해석 단계 풀이. sector 탭은 ① 매크로별 수치 + 신호 + *그 값이 나온 실물 원인* (PMI=설문 응답 비중, 금리차=장단기 수익률 차이, 금융환경=신용 스프레드·유동성, 실업청구=노동 수요) 인과 풀이 추가. (2) _FEATURE_LABEL_KO dict 30개 항목 + _build_indicator_text + _build_explain_text 의 "이성 점수"/"이성적"/"감정적" → "펀더멘털 반영도"/"정렬"/"분리" 일괄 변경. (3) static/js/main.js — _aiTitlesByIdx ['시장 심리','평소 이탈도','펀더멘털','종합'] → ['심리','시장 이탈도','펀더멘털 반영도','경기국면']. _aiIconSet 4 entry keys 갱신. _buildFgNoiseInsight 의 '이성적 상태'/'감정적 상태' → '정렬 상태'/'분리 상태'. KR 카드 제목 '시장 이성 점수' → '펀더멘털 반영도'. _cleanEmoji titles + titleKeys 배열 2곳 갱신. (4) static/js/i18n.js — nr.fundamental/price/match/gap, nr.sub.* 4국면 설명, section.nrChart, detail.noiseScore/noiseComposition, chart.yTop/yBottom, featDesc.realized_vol 등 "이성/감정" 사용 위치 모두 "정렬/분리" 또는 "펀더멘털 반영도" 로. (5) templates/about.html — "이성적 vs 감정적" → "정렬 vs 분리", "감정 거래 비중" → "단기 노이즈 거래 비중", "이성·노이즈" → "펀더멘털 정렬·분리". (6) templates/stocks.html — main.js?v=155→156 캐시 버스트. (7) api/routers/macro.py — _VAL_SIG_PROMPT 자문 표현(분할 매수 검토/방어 비중 확대 검토 등) 가드 강화 + build_valuation_interpretation 룰 fallback 친근 톤. 검증: 8 endpoint 응답에 "이성/감정" 단어 0건, "[결과 요인]" 라벨 일관, sector 탭 각 매크로별 인과 풀이 노출 확인. 회귀: DISABLE_GROQ=true 가드로 옛 LLM 캐시 우회 + 매번 fresh rule-based. 라벨 변경된 frontend keys 배열은 backend 새 라벨과 정확 매칭.)

마지막 갱신 시점: 2026-05-11 (시황 AI 해설 아이콘 블랙·오렌지 미니멀 + 펀더멘털 탭 배치 재구성. 사용자 스크린샷 + "AI 해설 앞 이모지 미니멀하고 세련된 블랙·오렌지 계열로, 펀더멘털 탭 UI 배치 새로". (1) static/js/main.js _aiIconSet — 옛 4색(블루/그린/오렌지/보라) 다채 이모지(◈/▲/◎/★) → 단색 톤 모노 geometric 심볼로 통일: 종합 ■ #FF8C00 솔리드+검정 fg(최강조), 심리 ◇ 오렌지 outline+10% tint, 평소 이탈도 ↗ 동일 tint, 펀더멘털 ◉ 동일 tint. 키워드 매칭 순서 재정렬 — '종합' 을 배열 최상단(idx 0)에 두어 종합 라인이 본문에 '방향 추정' 같은 문구 포함해도 '방향' 키워드보다 먼저 매칭(옛 버그: 종합 라인이 ▲ 로 잘못 매칭). 평소 이탈도/이탈도/이상도 키워드 추가, sentiment/rationality/signal 영문 키워드 보강. (2) static/css/main.css .ai-badge — 22→24px, font 11→13px, weight 800→700(geometric 심볼은 굵을수록 일그러짐), line-height 1, font-family monospace 추가(symbol 일관 렌더), margin-right 8→10. (3) templates/stocks.html 펀더멘털 탭 — 옛 [통합 카드(regime+chart)] + [분석 노트] 2블록 → [헤로 스코어 chromeless] + [차트 카드 chrome 1] + [분석 노트 chromeless] 3블록 위계. .fundamental-hero (= 옛 .fc-regime, .card-tappable JS 부착 호환 위해 .fc-regime 클래스 유지) → padding 18/4/14, border-bottom hairline, .fundamental-hero-head (overline + nr-badge flex space-between), nr-name 17→24px·weight 700→800, nr-icon-box 40→44, gap-track 4→6, nr-sub 12→13. .fundamental-chart-card 별도 .card chrome (padding 16/16/14 + .fundamental-chart-head). 분석 노트 .ai-explain-plain 그대로. (4) static/css/main.css 옛 .fundamental-combined/.fc-divider/.fc-chart 룰 삭제, 신규 .fundamental-hero/.fundamental-hero-head/.fundamental-hero-overline/.fundamental-chart-card/.fundamental-chart-head 룰 추가. .fundamental-hero.card-tappable padding-bottom 32→36 + ::after bottom 14→12 right 18→4 (chromeless 헤로에 맞춤). 검증: HTML 1섹션 재구성. CSS 룰 교체. JS _aiIconSet 4 entry + click handler 무관. 회귀: regime click → openDetail 그대로(.fc-regime 클래스 보존), nr-chart 렌더 그대로(getElementById 만 사용). 캐시 버스트 main.css?v=135→136, main.js?v=138→139, about.html 동기.)

마지막 갱신 시점: 2026-05-11 (시황 AI 해설 — 4줄 idx 기반 타이틀/아이콘 강제 (펀더멘털 중복 표시 + 인사이트 누락 버그 수정). 사용자 보고 "시황탭 AI 해설에서 인사이트 부분없이 펀더멘털 부분만 2개 나옴". 원인: LLM(qwen) 이 4번째 '종합' 라인을 가끔 "펀더멘털 — ..." 로 잘못 시작 → 옛 _matchIcon 키워드 매칭이 '펀더멘털' 키워드 hit → ◉ 펀더멘털 아이콘 + "펀더멘털" 타이틀로 렌더 → 화면상 펀더멘털 라인 2개, 종합 라인 사라짐. 해결 (frontend post-process, 백엔드 prompt 는 이미 가드 있어 무수정): (1) static/js/main.js _aiIconSet 재정렬 — 옛 [종합/심리/이탈도/펀더멘털] 순서를 출력 위치 순으로 [심리/이탈도/펀더멘털/종합] 으로 변경. 신규 상수 _aiTitlesByIdx = ['시장 심리','평소 이탈도','펀더멘털','종합'] 추가. (2) _matchIcon(text, idx, totalLines) 시그니처 +totalLines — totalLines===4 이고 idx 가 0~3 이면 _aiIconSet[idx] 강제 (키워드 매칭 우회). 다른 줄 수면 옛 키워드 매칭으로 폴백 (회귀 X). (3) _buildLineHtml(clean, icon, textContent, idx, totalLines) — totalLines===4 이면 forcedTitle = _aiTitlesByIdx[idx] 부여, 본문 시작이 옛 타이틀 prefix(예: '펀더멘털 — ')면 strip. (4) _formatAiText + loadAiSummary 의 nextLine() 두 렌더 경로 모두 totalLines 전달 + forcedTitle 적용. 백엔드 prompt(line 304) 의 '**반드시 "종합" 으로 시작**' 가드는 이미 존재 — frontend 가 LLM 가끔 빠뜨릴 때를 안전망으로 잡음. 검증: idx 기반 매핑이 발동하려면 lines.length===4 — 시황 4줄 구조는 prompt 가 enforce 하므로 안정적. 캐시된 옛 LLM 응답도 frontend 후처리로 즉시 정정 (DB 캐시 무효화 불필요). 캐시 버스트 main.js?v=140→141.)

마지막 갱신 시점: 2026-05-13 (AI 해설 — LLM 의존 제거 + 룰베이스 친근 톤 재작성. 사용자 요청 "LLM 안 쓰고 상세 요인들이 지금 탭 데이터에 어떤식으로 영향을 주고 있는지 이해하기 쉽게 간단하게". (1) api/routers/market_summary.py — _fallback_ai_summary KO 분기 전면 재작성: 옛 4줄 (시장 심리/이탈도/펀더멘털/종합 + 이모지) → 신규 3줄 [오늘 한눈에]·[왜 이런 결과]·[현재 구간], 이모지 0, 각 변수가 결과에 어떻게 기여했는지 풀이. _fallback_ai_explain 5탭(fundamental/signal/sector/sector-val/sector-mom) KO 분기 모두 [오늘 한눈에]/[왜 이 결과]/[참고] 3블록 친근 톤으로 재작성. 옛 [데이터 요약]/[주요 변수 설명]/[인사이트] 학술 톤 폐기. 자문 표현 0, 미래 방향 추정 0. (2) get_ai_summary/get_ai_explain/get_home_headline 3 endpoint 모두 DISABLE_GROQ=true 가드 추가 — 옛 LLM 캐시(in-memory/DB) 우회하고 매번 fresh rule-based fallback. background_tasks LLM 호출 등록도 차단. (3) get_home_headline 신규 fallback — _fallback_ai_summary 첫 2줄 사용. (4) api/routers/macro.py — build_valuation_interpretation 룰베이스 fallback 친근 재작성, 옛 "분할 매수 검토 / 방어 비중 확대 검토 / 비중 확대 검토 권장" 자문 표현 모두 제거. _VAL_SIG_PROMPT 도 자문 가드 강화 (액션 제안 표현 절대 금지). get_valuation_signal endpoint 에 DISABLE_GROQ 가드 — 옛 cache + DB interpretation 둘 다 무시하고 fresh rule-based 강제. (5) .env — LOCAL_GROQ_ONCE 제거, DISABLE_GROQ=true 유지. 검증: 8개 endpoint 모두 cached=False/source=fallback 응답 확인, 이모지 0/자문 단어 0/방향 추정 0. 회귀: Railway 배포 측 DISABLE_GROQ 미설정 환경에선 옛 LLM 경로 그대로 (가드 if 분기로 미영향).)

마지막 갱신 시점: 2026-05-13 (탭 헤드라인 — 8개 탭 최상단 룰베이스 한 줄 해설 추가 + 풀어쓴 표현. 사용자 요청 4 옵션 선택(룰베이스/오늘값+평소비교/탭 헤더 박스/이벤트 트리거 + 한국어만 + 면책 X + 8개 모두) → 추가 요청(이모지 X, 이성점수 노출 X, 박스 보라→오렌지, 데이터 의미 풀이형). (1) processor/tab_headline.py 신규 — 8개 룰 함수 + compute_all(region) + precompute_tab_headlines(region) + fetch_tab_headlines(region). 풀이형 표현: RSI → 상승속도, noise_score → 실적-주가 정합/괴리, D² percentile → N영업일에 한 번꼴 빈도, phase_name 4단계 → 자연어, VIX/VKOSPI → 공포지수, z_comp → 평소 평균 대비 비싸다/싸다, per_z → 비싼/싼 섹터. 임계값: chart yhat ±1%, market RSI 70/60/40/30 5단계, fundamental noise_score >1/>0/>-2/else, signal top_pct ≤5/≤10/≤30/else, sector VIX 평소 ±30%/-20%, sector-val per_z >1/<-1, sector-mom 1위+꼴찌, market-valuation z_comp ≥1/≥0.3/±0.3/≥-1/<-1. 이모지·이성점수 노출 0. 좌측 칩 시간 라벨이라 본문 오늘 prefix 제거. (2) api/routers/market_summary.py — /api/market-summary/tab-headline?region=... 엔드포인트 추가 (app_cache 우선 + live 폴백). (3) scheduler/job.py Step 18 추가 — precompute_tab_headlines(us). (4) scheduler/job_kr.py 17) 추가 — precompute_tab_headlines(kr). (5) templates/stocks.html — 8개 main.content tab-X 최상단에 div.tab-headline-box 삽입. 캐시 버스트 main.css?v=136→137, main.js?v=150→151. (6) static/css/main.css — .tab-headline-box (브랜드 오렌지 #FF8C00 좌측 강조선 + 연한 오렌지 배경, NEW 배지·AI차트 토글과 일관) + .tab-headline-label (오렌지 칩) + .loading opacity 룰 신규. (7) static/js/main.js — IIFE initTabHeadlines() append: DOMContentLoaded 1회 fetch withRegion(/api/market-summary/tab-headline) → 8개 div 채움. region 토글은 region.js location.reload() 가 처리. event-driven: 스케줄러가 데이터 새 계산 후 룰 재실행 → app_cache 1행 갱신. 자문 회피: descriptive only. KR 차트 모델 미학습 시 자동 폴백 메시지. 검증: 4 py 파일 ast.parse OK, main.js Node syntax OK.)


마지막 갱신 시점: 2026-05-16 (시장 밸류 탭 → 상승·하락 신호 탭 복구. 사용자 요청 "상승하락 확률의 차이 + 오늘 변동성 색 구분 + 과거 실제 가격과 이중축 비교해 선행성 시각화". (1) templates/stocks.html — #tab-market-valuation 컨테이너 cs-/cs- ID 가 tab-signal 의 cs-chart/cs-summary 와 충돌하므로 mv-cs-summary / mv-cs-volatility / mv-cs-chart-card / mv-cs-chart 로 prefix 분리. home.js?v=53→54 캐시 버스트. (2) static/js/home.js — loadMarketValuation 전면 재작성. region 별 SPY/069500 + VIX/VKOSPI 라벨 분기. 4-콜 Promise.all (/api/crash-surge/current, /api/crash-surge/history?days=90, /api/macro/latest, /api/chart/ohlc?ticker=…&interval=1d). ① hero: surge−crash 차이값 42px 큰 숫자 + 색 (>5 녹색, <-5 빨강, 그 외 회색) + 라벨 "상승 우세/약우세/균형/하락 약우세/하락 우세" + 우측에 surge/crash 분리 표시. ② 변동성 칩: VIX 값 → 5단계 색 (15 미만 녹/15-20 연녹/20-25 황/25-30 주황/30+ 적) + 한국어 라벨 (낮음/약낮음/보통/높음/매우 높음) + 구간 설명. ③ 이중축 SVG 차트 (720×260): 좌축 가격(녹색 라인) + 우축 (surge−crash) 0대칭 신호(주황 라인) + 0선 점선. 가격·신호 동일 날짜만 매칭(최근 90 거래일), 시작/중간/끝 X 라벨. 라이브 매칭 < 5건이면 "데이터 부족" 메시지. 탭 헤드라인 (#headline-market-valuation) 도 차이값·라벨로 갱신. 옛 ERP/Fed-Model 5/6-comp 코드 (renderGauge/arcPath/renderCompositeHistory/_xLeftLabel + loadMarketValuation 본문 ~200줄) 전부 삭제. _mvDays 전역도 제거. [검증] node --check home.js OK, 콜라이딩 ID 확인 (cs-chart 는 tab-signal 전용으로 단일화). [제한] 사용자 요청 따라 커밋·푸시 보류.)

마지막 갱신 시점: 2026-05-16 (상승·하락 신호 탭 — 10년 백필 + 방향성 정확도 % + 가격 20일 앞당김 + 기간 토글. 사용자 요청 "백필 통해서 10년 데이터 채우고 다시 구현. 방향성 예측 정확도 %로". (1) scripts/backfill_crash_surge.py — BACKFILL_DAYS 상수 → argparse --days (int 또는 "all"). 진행 카운터 100건마다 출력 + skip 예외 처리. (2) scripts/backfill_crash_surge_kr.py 신규 — 학습 없이 기존 models/crash_surge_xgb_kr.pkl 로드 → fetch_crash_surge_raw_kr (start 인자) → compute_features_kr → df_full dropna → backfill_crash_surge → upsert_crash_surge(region='kr'). train_kr_crash_surge 와 달리 재학습 안 함. (3) Windows 콘솔 cp949 호환 — 실행 시 PYTHONIOENCODING=utf-8 환경변수 필수 (스크립트 ═/— 문자 포함). (4) static/js/home.js loadMarketValuation — 기간 토글 (10년=2520 / 1년=252) + interval 분기 (≥1000 → '1wk', else '1d') + leadBars (주봉 4 / 일봉 20). 신호·가격 매칭 루프에서 currentPrice/futurePrice 둘 다 추출, 강한 신호 (|diff|>3) 중 실제 방향 일치 카운트 (nUp/hitUp/nDown/hitDown). 정확도 3종 산출: 전체 (hitUp+hitDown)/nStrong, 상승 우세 정확도 hitUp/nUp, 하락 우세 정확도 hitDown/nDown. (5) renderAccuracyBox(accAll, accUp, accDown, nStrong, nUp, nDown, leadLabel) 함수 — 3-column 카드 (전체/상승/하락), 60%+ 녹색 / 50%+ 주황 / 미만 빨강 색구분. 차트 SVG 위에 prepend. (6) lookupSignal helper — 캔들 날짜에 신호 없으면 최대 maxBack (주봉 7일 / 일봉 3일) 뒤로 walk-back 매칭. (7) X 축 라벨 full date (YYYY-MM-DD), font 13px bold; 범례 17px bold·색 칩 28×7; 라인 stroke 4.5 round join/cap. SVG viewBox 960×440 (max-height 560px). (8) templates/stocks.html home.js?v=58→59. [검증] node --check OK. US backfill 1차 cp949 실패 후 PYTHONIOENCODING=utf-8 재실행, KR 1차 동일 실패 후 utf-8 재실행. [제한] 사용자 지시대로 커밋·푸시 보류. KR 모델 학습 30 trials 진행 중 — 완료 시점 후 KR 10년 모드에서 정확도 박스 채워짐.)

마지막 갱신 시점: 2026-05-16 (UX 4건 — 외국인 순매수 단위 자동 스케일 + KNN 이벤트 라벨 가까운 시기 폴백 + 거시경제 둔화 헤드라인 top-3 요인 + My Holdings 4국면 평균 폴백. 사용자 요청 4개를 한 번에. (1) static/js/main.js (KR P/C 카드 자리의 외국인 5d 순매수) — |억| ≥ 10,000 (= 1조) 이면 조 단위 + 소수 1자리, 미만이면 억 단위 + 정수. 임계값도 단위에 맞춰 ±1조 (10,000억) 기준: 강한매수/매수우위/매도우위/강한매도 4단. 색상은 부호 기반 유지. 6.10조 → "+6.1 조" 표기, -19.9조 → "-19.9 조" 표기 자연스러움. (2) static/js/anomaly.js findEvent 리팩토링 — 옛 반환 string|null → {label, exact, offset}. 1단계 range 직접 매칭은 그대로, 2단계로 가장 가까운 사건을 시간 거리 최소화로 선택 (dFrom>0 미래, dTo<0 과거, edge 거리 기준). _dayDiff 헬퍼 + _offsetTxt 헬퍼 (≤14일=일/≤90일=주/≤730일=개월/그 외=년) 신규. renderKnn 에서 exact 면 📌 + 정상색, 인접 매칭이면 📎 + 회색 + 오프셋 + 라벨 ("3개월 후 · 일본 캐리트레이드 청산·VIX 65 폭등" 같은 형태). 안내 텍스트 "📌 = 해당 시점 주요 사건. 📎 = 시간상 가장 가까운 사건 + 오프셋" 으로 갱신. 옛 "특별한 시장 이벤트 없던 평소 시점" 문구 완전 제거. region 분기 (MARKET_EVENTS_US/KR) 동작은 유지. (3) processor/tab_headline.py _headline_sector 재작성 — 옛 region 별 spec 3개 고정 → 국면별 우선순위 리스트 _PHASE_DRIVERS_US (회복/확장/둔화/침체) + _PHASE_DRIVERS_KR. _MACRO_META 신규 (key→라벨/단위/부호). _fmt_macro 헬퍼로 라벨 + 값 + 단위 조립. 우선순위 순서대로 snap 조회 → null 스킵 → top-3 추출. 1순위는 가장 큰 요인으로 명시: "{phase_meaning} 국면. {macro_phrase} 중 {top_label}이(가) {phase} 국면의 가장 큰 요인으로 작동하고 있다." 형식. US 회복 sample 출력: "...회복 국면. PMI 3M변화 -0.25, ANFCI -0.48, 설비투자3M -24.27% 중 PMI 3M변화이(가) 회복 국면의 가장 큰 요인". KR 둔화 sample: "...둔화 국면. 장단기금리차 +0.33%p, 설비투자YoY +8.44%, 건축허가YoY -24.03% 중 장단기금리차이(가) 둔화 국면의 가장 큰 요인". (4) static/js/sector.js 보유 종목 성과 — currentPhase 의 row 가 사용자 보유 종목과 교집합 0 일 때 4국면 평균으로 폴백. phases ['회복','확장','둔화','침체'] 각 ticker 값 평균 산출 → avgRow. 헤더 텍스트 "4국면 평균 ~수익률". 평균조차 산출 못 하면 옛 sector.holdNoData 메시지 유지. (5) templates/stocks.html 캐시 버스트 main.js?v=164→165, sector.js?v=9→10, anomaly.js?v=22→23. [검증] py_compile tab_headline.py OK + node Function() anomaly/sector/main.js OK. [edge case] 외국인 fnb5=null → "데이터 부족" 유지, KNN events 사전 비어있을 때 ev=null → eventHtml 생략, sector phase=='?' → drivers map miss → "{phase} 국면이다." 폴백, holdings 4국면 전부 null → 옛 메시지 유지. [제한] 커밋·푸시 보류 (사용자 지시).)


마지막 갱신 시점: 2026-05-19 (AI차트 탭 — Coinbase Prices 스타일 list 뷰 + sort/search + sparkline draw 애니메이션. 사용자 요청 "AI차트 = 가격 리스트 뷰 + 상세 토글, XL시리즈/ARKK 까지 포함, 시가총액/거래대금/등락률 정렬, search 기능, sparkline 차트 그려지는 애니메이션". (1) static/js/chart.js — CHART_MAIN_TICKERS_US 9개 → CHART_TICKERS 16개 (SPY/QQQ/DIA/IWM/VTI/VOO/SOXX/SMH/XLK/XLF/XLE/XLV/ARKK/GLD/TLT/SCHD) 확장. TICKER_NAMES 에 VTI/XLK/XLF/XLE/XLV/ARKK 라벨 추가. window.selectChartTicker(t) 신규 export — chip 없는 ticker 도 직접 _chartTicker 갱신 + loadCandleChart() 호출 (AI차트 list 클릭 시 backend /api/chart/ohlc 16개 ETF 지원). (2) static/js/home.js — AI_TICKERS_US/AI_TICKERS_KR 정의 (US 16, KR 11, chart.py CHART_TICKERS 매칭). getAiTickers() region 분기. showAiChartList/showAiChartDetail 토글 + _aiPlayAnim 헬퍼 (in-right/out-left 등 슬라이드 전환 4종). buildAiChartList — row 마다 fade-up 위→아래 stagger 60ms 증분 (max 600ms). 16 ETF fetch /api/chart/ohlc 으로 last/pct/value (last×volume) 계산 → _aiMetrics 캐시. polyline 에 points set 직후 인라인 shorthand animation: ai-spark-draw 1.3s cubic-bezier(.2,.7,.2,1) {delayMs}ms forwards 재정의 (delayMs = 200 + idx*50, max 500). row idx 별로 행 stagger 끝나는 시점에 sparkline 좌→우 1.3s draw. (3) 초기 표시 + "모두 보기" 토글 — AI_LIST_INITIAL=10. idx ≥ 10 row 는 aichart-hidden 으로 숨김. 버튼 클릭 시 _aiListExpanded 토글 + 숨겨진 row 재 fade-up stagger. (4) 정렬 dropdown — AI_SORT_LABEL {mcap, value, up, down}. AI_MCAP_HINT 룩업 (US ETF AUM B$ 16개 / KR ETF AUM B₩ 11개). _sortedTickers() + _reorderAiList() DOM appendChild 로 순서만 재배치 (재 fetch 안 함). #ai-sort-trigger 클릭 → #ai-sort-menu 토글, li 클릭 → mode 설정 + 라벨 갱신 + 외부 클릭 시 닫힘. (5) Search input — #ai-search-input 텍스트 input. delegated input 이벤트 → _applyAiSearchFilter() 가 ticker 또는 종목명 substring 매칭, 비매칭은 aichart-filtered 로 hide. 검색 중 모두 보기 버튼 숨김 + 결과 0건이면 ""q" 결과 없음" 메시지. 검색 해제 시 _aiListExpanded 상태로 hidden 복원. (6) cb-top 헤더 — 선물/알림 .cb-icon-btn 2개 삭제. .cb-search 를 label + input 으로 변경. .cb-filter-pill 을 .cb-sort-wrap dropdown 으로 교체. (7) sparkline keyframe — 기존 ai-line-draw (dashoffset 600→0, dasharray 200 sparkline 에서 3 cycle 깜빡임 발생) → 신규 ai-spark-draw (200→0, 단일 cycle) 추가. .cb-tk .cb-spark polyline 에 적용. (8) KR_SUPPORTED_TILES set 에 crash-surge 추가 — KR 모드에서 상승·하락 신호 타일 활성화 (DB region=kr 데이터 이미 적재됨). (9) loadCrashSurge hero 라벨 "향후 한달뒤 상승 확률 − 하락 확률" + 변동성 "향후 한달간 변동성 예상" 라벨 변경, 정확도 박스 제거. tab-crash-surge 헤드라인 badge 삭제. (10) 캐시 버스트 chart.js?v=35→37, home.js?v=53→65, main.js?v=163→164 (intro 제외), templates/stocks.html 인라인. [검증] node --check chart.js/home.js OK. backend /api/chart/ohlc XL 시리즈 4개 응답 OK (chart.py CHART_TICKERS 16개 검증). /api/chart/similar 도 16개 모두 cron precompute 적재. /api/crash-surge/current?region=kr 응답 OK (068219 row 2026-05-18). [제한] intro 안내 화면은 본 커밋에서 제외 (사용자 요청).)
